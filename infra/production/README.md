...
再提交 8 个长回复任务，在数据库观察到运行中租约后用 `SIGKILL` 等价方式强制终止 Worker，要求
新进程在 300 秒内通过过期租约接管、至少一条任务 `attempts >= 2`、全部完成且无失败/活动积压；
该时限覆盖 60 秒租约和百炼单次 60 秒、最多 2 次客户端重试的配置上界，并保留队列调度余量。
最后注入 active API 故障并回滚。k6 是后续容量和 SLO 复验的长期工具，macOS 本机缺少时直接执行
`brew install k6`。

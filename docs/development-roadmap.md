...
| 本地服务 | `✅` S10-06 验收后 demo/real API、Worker、前端、平台/RAGFlow/恢复容器、浏览器和项目专属 Colima 已停止；正式稳定 Volume、0600 Token、现有 Demo Key 与官方镜像保留，隔离演练数据已删除 |
| 后续整改 | `🟨` Wave 7、Wave 8、Wave 9、S10-01 至 S10-06 已完成；下一任务 S10-07 建立生产构建、远程部署与回滚，实际远程资源和首次发布仍须用户明确授权 |
...
正式页面验证真实上传文档、员工绑定与审计链，演练资源最终全清理。下一任务是 S10-07 生产构建、
远程部署与回滚；实际创建远程资源和首次发布仍须用户明确部署指令，本轮未执行。所有后续任务仍

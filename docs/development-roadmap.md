...
> 文档性质：项目开发进度唯一执行台账  
> 建立日期：2026-07-19  
> 当前阶段：S10-07E 用户模型管理已完成；下一任务 S10-07F 数字员工默认模型
...
| 本地服务 | `✅` 供用户联调的正式开发 API `18200`、前端 `18280`、平台 MySQL 与 RAGFlow v0.26.4 保持运行；本轮隔离 API/Worker/前端、无头浏览器、测试认证数据和截图已清理，稳定 Volume、0600 Token、现有 Demo Key、升级前认证备份与固定镜像保留 |
| 后续整改 | `🟨` Wave 7、Wave 8、Wave 9、S10-01 至 S10-07E、S10-07K 与 S10-07L 已完成；S10-07F 至 S10-07J 与 S10-07M 已排入并将在 S10-08 前逐项交付；尚未采购、创建或发布任何远程资源 |
...
| S10-07F | 数字员工默认模型 | 数字员工创建与编辑必须从当前租户已启用模型中选择默认模型并持久化引用；现有员工和默认员工通过可回滚迁移绑定平台默认模型，停用模型保留既有引用但禁止新选，删除仍被引用的模型关闭失败；通过正式员工页面和 API→MySQL→Deep Agents→百炼路径验证创建、编辑、刷新和实际回复所用模型 | S10-07E | ⬜ 未开始 |
...
设计规范、交互原型和强制 UI 规则，S10-07L 也已把已有正式页面统一到该设计语言。当前从
S10-07F 开始依次交付数字员工默认模型、无需员工且首次发送自动创建
的通用会话与逐轮模型切换、左侧历史会话恢复、可选员工或模型的工作流 AI 对话节点，以及知识库

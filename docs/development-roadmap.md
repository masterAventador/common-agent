...
> 文档性质：项目开发进度唯一执行台账  
> 建立日期：2026-07-19  
> 当前阶段：S10-07D 知识库人工验收样本文档已完成，下一任务为 S10-07E 用户模型管理
...
| 后续整改 | `🟨` Wave 7、Wave 8、Wave 9、S10-01 至 S10-07D 已完成；S10-07E 至 S10-07J 已按用户最新需求排入并将在 S10-08 前逐项交付；尚未采购、创建或发布任何远程资源 |
...
| S10-07E | 用户模型管理 | 新增租户隔离的模型配置领域、MySQL 迁移、引用安全 CRUD、审计、正式 API 和左侧“模型管理”页面；首版提供商为阿里百炼，用户维护显示名称、百炼模型标识和启用状态，业务选择器只读取用户已创建且启用的模型，不直接枚举百炼全部模型；通过正式页面创建、编辑、停用、引用阻断、刷新恢复和真实百炼调用验收 | S10-07D | ⬜ 未开始 |
...
| S10-08 | 生产安全、性能与最终验收 | 执行依赖/SAST/Secret/容器扫描、权限与输入攻击测试、并发/容量/长连接/故障恢复压测；定义并满足 SLO、资源预算和告警；从新租户通过正式浏览器完成模型→知识库→员工→会话→工作流并验证审计、备份与回滚 | S10-07J | ⬜ 未开始 |
...
连接或发布任何远程
资源，第一次远程发布仍须用户再次明确授权。S10-07D 已建立知识库人工验收样本；S10-07E 至
S10-07J 将依次交付用户模型管理、数字员工默认模型、无需员工且首次发送自动创建的通用
会话与逐轮模型切换、左侧历史会话恢复、可选员工或模型的工作流 AI 对话节点，以及知识库批量
选择/拖拽上传。下一任务为 S10-07E；上述任务全部通过后再进入 S10-08 生产安全、性能、SLO/告警
与最终验收。所有后续任务仍

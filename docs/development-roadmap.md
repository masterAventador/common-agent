...
> 文档性质：项目开发进度唯一执行台账  
> 建立日期：2026-07-19  
> 当前阶段：S10-08 生产安全、性能与本机最终验收已完成，待远程部署明确授权
...
| 前端入口 | `✅` React/Vite/Ant Design 六入口壳层已通过组件、构建和真实浏览器导航验收；模型管理对成员可见，审计入口仅对 Owner 可见 |
| 跨端契约 | `✅` FastAPI OpenAPI、前端生成 DTO 和隔离漂移检查已形成单一来源闭环 |
| 前端 API | `✅` Axios、Query Client、Zod、CORS 与后端真实成功/失败状态已跨端跑通 |
| RAGFlow 基线 | `✅` 官方 v0.26.4/tag commit `cb93883f3f8c975eecb2fed81210effeb3bdb06f`、common-agent-dev 隔离栈、loopback 端口、数据目录和资源策略已锁定 |
| 产品代码 | `✅` 知识库批量选择/拖拽上传、逐文件状态和解析重试，以及数字员工、通用/员工连续会话、全局历史恢复、工作流设计/运行及两类触发的 MVP 正式链路已完成；数字员工默认模型、通用会话当前模型、助手逐轮模型快照及工作流 AI 节点员工/模型执行目标均使用租户模型配置和正式引用 |
| 本地服务 | `✅` 正式开发 API `18200`、Worker、前端 `18280`、平台 MySQL 与 RAGFlow v0.26.4 已完成本轮联调和验收；正常开发库位于 `20260722_0022`，当前服务恢复为验收前停止状态，稳定 Volume、0600 Token、现有 Demo Key、升级前认证备份与固定镜像保留 |
| 后续整改 | `✅` Wave 7、Wave 8、Wave 9、S10-01 至 S10-08 全部完成；本机范围无未完成整改，尚未采购、创建或发布任何远程资源 |
...
| Redis/消息队列 | 不可用、超时、重复投递、乱序、积压、消费失败和恢复 | ✅ S10-05 不引入 Redis/MQ，使用平台 MySQL 持久队列与事件端口；连接失败、并发重复领取、租约过期、有限重试、积压统计、顺序恢复、幂等键和容量/保留边界均已覆盖，RAGFlow 内部 Valkey 仍不作为平台接口 |
| 对象存储 | Bucket/权限错误、上传中断、重复对象、清理失败和容量上限 | ✅ 平台仍不直连对象存储；文档只经 RAGFlow 正式 API。S10-06 将其内部 MinIO 与 MySQL/Elasticsearch/Valkey 一起停写冷备，独立恢复后由正式知识页验证原上传文档，仍不绕过 RAGFlow 作为业务接口 |
| Worker | 启动失败、任务丢失、超时、重试幂等、崩溃恢复和优雅停止 | ✅ 独立 Worker 入口、租约/随机栅栏、心跳续租、持久停止信号、有限重试、崩溃接管、处理器取消与优雅停止均已覆盖；API 只提交任务，不再用进程内后台任务冒充正式执行 |
| RAGFlow | 未启动、Key 错误、超时、知识库不存在、供应商注册失败和 API 版本漂移 | ✅ `adapters/knowledge/test_ragflow.py` 与模型配置契约故障注入定位，真实 v0.26.4 生命周期/公开 HTTP/会话检索验收；新供应商 API 对不支持模型关闭失败并改用同一百炼的官方兼容端点，正式适配层统一安全错误 |
| 文档 | 空文件、超限、不支持类型、重复上传、解析失败和解析超时 | ✅ `knowledge/test_service.py`、批量队列与 RAGFlow 重复/上传未知结果/状态映射测试覆盖空文件、20 MiB、20 份上限、同名同大小去重、并发上限、部分失败和单项重试；正式页面经平台 API→真实 RAGFlow v0.26.4 验收 12 份混合文档、真实解析中断/重试和跨文档召回，900 秒 deadline 继续覆盖解析超时 |
...
| 备份/恢复 | 缺密钥、弱权限、篡改/截断、清单漂移、停写失败、组件缺失、非空目标、超期保留、RPO/RTO 超标和演练污染 | ✅ `test_archive.py`、`infra/backup/test-manage.sh` 与 `manage.sh drill` 覆盖 AES-256-GCM 认证失败、逐文件 SHA-256、符号链接/覆盖拒绝、30 天且至少 7 代保留、API/Worker/RAGFlow 停写、四卷与外部引用完整性、`common-agent-recovery-*` 空目标；独立源销毁后 63 秒恢复并由正式页面验证真实文档、员工绑定和审计链，最终资源全清理 |
...
| S10-05 | 持久任务、事件与 Worker | 通过平台自有任务/事件端口引入真实队列和 Worker；会话回复、工作流运行、停止、幂等、重试、积压、崩溃恢复和至少一次投递语义明确；SSE 可从持久序列恢复，多 API/Worker 实例不重复执行副作用 | S10-04,H7-07 | ✅ 已完成 |
| S10-06 | 备份、恢复与灾难演练 | 为平台 MySQL、对象/上传数据、RAGFlow 外部引用和部署配置定义备份、加密、保留、恢复点与恢复时间；从独立备份恢复到空环境并由正式页面验证核心数据与引用，演练不污染正式数据 | S10-05 | ✅ 已完成 |
| S10-07 | 生产构建、远程部署与回滚 | 建立最小权限镜像、不可变版本、环境配置、TLS/域名、数据库迁移、健康/就绪、灰度与回滚流水线；先在隔离环境完成生产同路径。实际创建远程资源和首次发布必须再次取得用户明确部署指令 | S10-06 | ✅ 已完成 |
| S10-07A | RAGFlow v0.26.4 维护升级 | 把官方 submodule、镜像和版本契约原位升级到 v0.26.4；先做认证冷备，保留原生 Volume，验证官方 schema/供应商迁移、百炼 embedding/rerank、现有数据、恢复、生产蓝绿与完整本地链路；禁止修改上游源码、镜像内文件或数据库 | S10-07 | ✅ 已完成 |
| S10-07D | 知识库人工验收样本文档 | 在项目内建立独立上传语料与不上传的标准答案，提供不少于 12 份、提取正文至少 2 万中文字符且带唯一事实标记的 TXT、Markdown、多页 PDF、多页 DOCX；内容须覆盖精确事实、语义改写、相近指标、版本冲突、临时例外、跨文档多跳与无答案问题，不得用重复填充凑字数；二进制文件必须完成结构、文本提取、页数和无窗口逐页渲染检查，样本不得含真实凭据、用户数据或伪装成产品规则 | S10-07C | ✅ 已完成 |
| S10-07K | 设计基线与项目 UI 规则接入 | 将用户提供的 `DESIGN.md` 与自包含 HTML 交互原型原样纳入 `docs/design/` 并校验来源哈希；项目规则强制所有新增与改造 UI 遵守该规范，明确 HTML 只作布局/交互/代码参考、不得替代正式 React 或定义业务语义；允许复用“负荷曲线 + 蓝色火花节点”动效 Logo，但不得照搬 `PowerAI` 名称、电力行业定位、示例账号或数据。通过离线无头浏览器打开仓库副本并遍历对应页面，确认资料可用且不启动可见窗口 | S10-07D | ✅ 已完成 |
| S10-07L | 现有页面统一设计改造 | 建立集中 Design Token、统一 App Shell、Lucide 图标、状态/卡片/表单组件与使用本项目名称的动效 Logo；按 HTML 中已有对应 UI 改造正式认证、聊天、数字员工、知识库、工作流列表与设计器，审计等无对应原型页面仍按 `DESIGN.md` 统一，不引入原型中的团队空间、执行监控或电力行业假业务。保留 React、Ant Design、权限、响应式、键盘和真实 API 语义；组件测试覆盖状态，正式无头浏览器在桌面与窄屏逐页截图并完成真实功能回归 | S10-07K | ✅ 已完成 |
| S10-07E | 用户模型管理 | 新增租户隔离的模型配置领域、MySQL 迁移、引用安全 CRUD、审计、正式 API 和左侧“模型管理”页面；首版提供商为阿里百炼，用户维护显示名称、百炼模型标识和启用状态，业务选择器只读取用户已创建且启用的模型，不直接枚举百炼全部模型；页面遵守 `docs/design/DESIGN.md` 并参考 HTML 模型管理布局，但不得照搬内置供应商目录；通过正式页面创建、编辑、停用、引用阻断、刷新恢复和真实百炼调用验收 | S10-07L | ✅ 已完成 |
| S10-07F | 数字员工默认模型 | 数字员工创建与编辑必须从当前租户已启用模型中选择默认模型并持久化引用；现有员工和默认员工通过可回滚迁移绑定平台默认模型，停用模型保留既有引用但禁止新选，删除仍被引用的模型关闭失败；通过正式员工页面和 API→MySQL→Deep Agents→百炼路径验证创建、编辑、刷新和实际回复所用模型 | S10-07E | ✅ 已完成 |
| S10-07G | 通用会话、自动建会话与逐轮模型切换 | 点击 AI 会话直接进入无需数字员工的空白通用对话；用户无需预建会话，首次发送时由正式入口原子创建会话并提交首轮。输入框底部只显示用户已创建且启用的模型，可在每轮发送前切换；会话持久化通用/数字员工来源和实际逐轮模型，通用会话恢复上次选择，员工会话中的临时模型切换只影响当次/当前打开期间，不改员工默认模型 | S10-07F | ✅ 已完成 |
| S10-07H | 左侧历史会话与恢复语义 | 在现有左侧主菜单下增加分割线和按更新时间稳定排序的历史会话区，显示会话标题及“通用 AI”或数字员工归属；点击直接加载完整历史并可继续对话，刷新、分页/加载更多、删除和资源失效均有明确状态。重新进入员工会话时始终回到该员工当前默认模型，即使此前逐轮临时选择过其他模型；通用会话恢复其持久化模型 | S10-07G | ✅ 已完成 |
| S10-07I | 工作流 AI 对话节点执行目标 | 工作流 AI 对话节点提供互斥执行目标：选择一个已创建数字员工，或直接选择一个用户已创建且启用的模型；员工目标继承其系统提示、知识库、授权工作流和当前默认模型，模型目标执行通用 AI 对话。服务端保存/编译时校验租户、启用状态和引用，运行摘要记录实际目标与模型；通过正式拖拽编辑、保存、手动运行和员工触发路径完成真实 LangGraph/Deep Agents/百炼验收 | S10-07H | ✅ 已完成 |
| S10-07J | 知识库批量选择与拖拽上传 | 知识库页面支持一次选择多个 TXT/Markdown/PDF/DOCX 和拖拽文件到上传区，显示逐文件等待、上传、解析、完成与失败状态，并支持单项重试；限制并发、类型、单文件大小和重复提交，部分失败不得回滚或重复成功项。通过正式页面批量拖入 S10-07D 样本，经平台 API→RAGFlow v0.26.4 完成解析、列表刷新和跨文档检索，浏览器全程无头且清理验收数据 | S10-07I | ✅ 已完成 |
| S10-07M | 全仓架构、性能与代码结构 Review 及整改 | 在全部产品增补功能完成后审查领域/应用/适配器依赖方向、模块职责和公共抽象、重复与复杂度、MySQL 查询与事务/N+1、异步并发和 Worker 租约、外部调用超时与资源释放、React 状态/渲染/包体、契约、安全和测试可定位性；结合仓库架构门禁、Ruff/Mypy/ESLint/TypeScript、依赖与安全扫描、复杂度/性能分析和逐模块人工审查建立问题台账。每个问题先保留可复现证据，再单独整改、定向回归并按问题提交；最终通过全量门禁和模型→知识库→员工→会话→工作流正式用户链复验，不以工具零告警代替真实验收 | S10-07J | ✅ 已完成 |
| S10-08 | 生产安全、性能与最终验收 | 执行依赖/SAST/Secret/容器扫描、权限与输入攻击测试、并发/容量/长连接/故障恢复压测；定义并满足 SLO、资源预算和告警；从新租户通过正式浏览器完成模型→知识库→员工→会话→工作流并验证审计、备份与回滚 | S10-07M | ✅ 已完成 |
...
持久化和投影职责拆分，并由体量、依赖方向、Feature 私有边界和循环依赖门禁保护；六路由异步
...
失败。S10-05 也已完成：会话回复和工作流运行由 API 原子写入 MySQL 持久任务，独立 Worker 以
租约、随机栅栏、心跳、种类隔离槽位和有限退避执行；停止意图、积压、崩溃接管、终态事件修复及
至少一次投递语义均已验证。会话/工作流 SSE 从按租户隔离的 MySQL 单调序列恢复，API/Worker
重启或多实例领取不会重复业务副作用。S10-06 已完成：平台 MySQL 逻辑快照、RAGFlow 四个冷备
Volume、外部引用与非敏感部署配置进入同一 AES-256-GCM 认证归档，24 小时 RPO、120 分钟 RTO、
30 天且至少 7 代保留和 90 天演练周期已有自动门禁；隔离源环境销毁后 63 秒恢复到空 MySQL/Volume，
正式页面验证真实上传文档、员工绑定与审计链，演练资源最终全清理。S10-07 也已完成：后端和
前端使用固定基础镜像 digest、非 root/只读文件系统与不可变 release；业务节点和 RAGFlow 节点
通过私网 HTTPS/独立私钥分离，业务节点蓝绿切流、显式向前迁移、正式入口验证与代码回滚已在
隔离 Colima 中完成真实 RAGFlow 和 Chromium 演练。S10-07A 随后把未修改的官方 submodule、
镜像与配置原位升级到 v0.26.4；升级前认证冷备、官方 schema/供应商迁移、同一百炼的两个兼容
实例、现有数据、独立恢复、生产蓝绿与 30 分钟完整 real 链路均已验收。当前没有采购、创建、
连接或发布任何远程
资源，第一次远程发布仍须用户再次明确授权。S10-07D 已建立知识库人工验收样本，S10-07K 已纳入
设计规范、交互原型和强制 UI 规则，S10-07L 也已把已有正式页面统一到该设计语言。
S10-07D 至 S10-07M 的产品增补、设计统一和全仓 Review 均已完成；S10-08 也已通过生产安全、
容量、SSE、Worker 崩溃恢复、SLO/资源/告警、正式新租户浏览器链、灾备和回滚终验。本机范围
当前没有未完成开发任务；下一步仅为用户明确发出“部署”指令后的远程资源采购、首次发布和
远程同路径复验。GitHub CI 仍只作可选镜像，不作为完成依据。
...
### S10-05 持久任务、事件与 Worker

- 状态：✅ 已完成
- 日期：2026-07-21
- 提交：本任务提交（见 Git 历史）
- RED：先为任务/事件平台模型、队列与 Journal 端口、MySQL 并发领取、API 原子入队、独立 Worker、
  会话/工作流执行、停止、退避重试、租约接管、随机栅栏、积压和持久 SSE 建立失败测试；最初因
  `tasks`、`events`、`worker_main`、`0016` 和正式适配器不存在而收集或断言失败。后续 RED 又准确
  捕获最后一次准备阶段异常未写业务失败、租约丢失后旧处理器仍可能写终态、所有槽位被会话父任务
  占满造成工作流子任务饥饿、已过期事件仍占用幂等键，以及标准 E2E 只启动 API 未启动 Worker。
  正式浏览器首轮还证明旧“断流后手工重试”预期已过时：Worker 已自动重试成功，页面不会进入失败态
- 任务与 Worker：新增平台自有固定 payload、`TaskQueue`、`TaskExecutionContext`、`TaskWorker` 与
  并发池，`0016` 建立 `durable_tasks`。会话发送/重试和工作流启动分别在原业务 Unit of Work 中
  原子写入占位摘要与任务，API 不执行模型或 LangGraph。MySQL 领取使用行锁跳过已占用记录并按
  处理器 kind 过滤；默认八槽按会话/工作流各四槽隔离，避免父会话等待子工作流时耗尽执行能力。
  至少一次投递由租约、心跳和有界指数退避实现，所有完成/失败/取消写入必须携带随机栅栏令牌；
  租约丢失会取消旧处理器但不伪造业务停止，进程退出后的任务由租约到期接管
- 停止、幂等与副作用：租户内幂等键保证同一消息/运行只入队一次，多 Worker 并发只能取得一个
  有效租约；停止通过持久 `stop_requested` 由心跳传播，用户停止与租约丢失分别收敛为取消和接管。
  会话重试清除上一尝试不完整内容，工作流重试清除部分节点进度；业务终态已提交但事件追加失败时，
  确定性事件键只补写终态事件，不再次调用模型、工具或工作流副作用。准备阶段异常在最后一次尝试
  同样持久化稳定错误码，活跃会话删除关闭失败
- 持久事件与 SSE：新增 `EventJournal` 与 `durable_event_streams/durable_events`，会话和工作流事件
  按租户、流类型、流 ID 分配单调序列，并以确定性事件键幂等追加。默认保留 30 天、单流最多
  100,000 条，过期历史不再可读且释放事件键；缺失序列关闭失败为历史不可用。API 内每订阅者最多
  128 条一批轮询、全局最多 1,024 个订阅者，`after_sequence`/`Last-Event-ID` 可跨 API、Worker
  和进程重启恢复，超出保留边界时回退 MySQL 权威快照
- GREEN：后端权威覆盖率全量 `617 passed, 13 skipped in 210.07s`，总体行 91.38%、分支
  72.73%、核心行 93.28%、核心分支 75.00%，全部超过冻结门禁；Ruff、287 文件格式、严格 Mypy
  270 个源/测试文件、uv lock、85 包安全审计、Alembic `0016` head/无模型漂移、OpenAPI/生成
  DTO、架构、CI/覆盖率/E2E 入口、ShellCheck、Secret 和两类基础设施门禁通过。前端 23 files/
  107 tests，覆盖率行 87.35%、分支 76.04%，ESLint、TypeScript、Build、零高危依赖审计、契约与
  五路由 bundle 门禁通过；最大 chunk 189,644 bytes，五路由首次 JS 图均低于 1.5 MB
- 生产同路径：标准 `demo-chat` E2E 现同时启动正式 FastAPI 与 `common_agent.worker_main`，12 GiB
  Chromium 完成员工/知识库创建、两轮带引用会话、第三轮断流、独立 Worker 自动重试、持久 SSE
  收敛和页面刷新后恢复，最终 `1 passed (11.5s)`，浏览器没有直连 RAGFlow 或百炼。正式 HTTP/
  MySQL 集成另覆盖独立 API/Worker、并发领取、停止、崩溃接管、终态事件补写和跨进程 SSE；本任务
  未调用付费 real 模型、未修改 RAGFlow submodule/镜像/容器内文件，也未执行远程部署
- 清理与文档：接管时发现的旧 demo-light launchd API/前端和卡住一小时的旧 pytest 均在确认
  工作目录属于本仓库后精确停止，没有触碰 `hug-ai`。E2E finally 已删除唯一会话、员工、知识库和
  认证状态；失败截图/Trace 用于定位旧预期后保留到最终清理，成功产物自动删除。最终停止 API、
  Worker、前端、平台 MySQL 与项目专属 Colima，稳定 Volume、冻结依赖、官方镜像、0600 Token 和
  用户授权的百炼 Demo Key 保留；README、工程结构、前后端架构、脚本说明与本路线图已同步
- 遗留：无；下一任务 S10-06 为平台 MySQL、上传/外部引用和部署配置建立可恢复的备份、加密、
  保留、RPO/RTO 与空环境灾难演练
...
- 日期：2026-07-21
- 提交：本任务提交（见 Git 历史）
- RED：先为认证加密归档、错误密钥、篡改/截断、文件清单与 SHA-256、密钥权限、符号链接、
  非空恢复目标和 CLI 建立单元测试，首次因 `common_agent.adapters.backup` 不存在按预期收集失败；
  基础设施契约也因 `infra/backup/manage.sh` 不存在失败。随后以隔离正式栈执行灾难演练，依次发现
  API/Worker 并发启动会争用迁移、MySQL socket 就绪早于应用 TCP 凭据就绪，以及正式审计页标题
  与测试定位器不一致，均保留真实失败原因后修复，没有跳过停写、源销毁、页面或 RTO 门禁
- 数据与归档：新增流式 AES-256-GCM 认证归档，独立 256-bit 十六进制密钥必须是普通 `0600`
  文件；认证头固定格式版本，压缩载荷包含规范 manifest、逐文件大小/SHA-256 和任务元数据，读取
  时先完成认证再安全解包。归档拒绝空源、符号链接、非普通文件、不安全路径、重复/缺失/多余文件、
  摘要不符、错误密钥、篡改、截断、已有输出和非空恢复目录，并以同目录临时文件原子发布
- 一致性与保留：应用 API、Worker 和前端必须停写；平台 MySQL 用 `--single-transaction` 逻辑
  快照，RAGFlow 停止后冷备 MySQL、MinIO、Elasticsearch、Valkey 四个专属 Volume；另存平台
  `tenant_id -> knowledge_base_id` 外部引用、Alembic 版本、非敏感 allowlist 配置和固定部署版本。
  百炼 Key、RAGFlow Token、会话/引导凭据、数据库口令与备份密钥不入归档。策略固定 24 小时
  RPO、120 分钟 RTO、30 天保留、至少 7 个代际和每 90 天一次演练；动态保留测试证明只删除同时
  超期且超过最小代际的归档
- 恢复与失败矩阵：`restore` 要求精确确认词、隔离 `common-agent-recovery-*` 名称、空 MySQL、
  四个不存在的恢复 Volume 和不存在的恢复目录，拒绝覆盖正式资源；归档必须同时具备平台 SQL、
  外部引用、四卷和部署配置。静态/动态门禁覆盖缺密钥、弱权限、策略非法、应用/Worker/RAGFlow
  未停、组件缺失、非空目标、保留边界与演练清理；恢复 Runbook 明确生产归档须异地保管且密钥
  分离，当前无 binary log 时恢复点为最近一次已验证归档
- 生产同路径：在专属 32 GiB Colima 中创建完全隔离的源平台 MySQL 与 RAGFlow v0.25.6，正式
  Chromium 经 React/FastAPI/RAGFlow 页面创建真实知识库、上传并等待解析 `generic-knowledge.txt`、
  绑定员工，种子阶段 `1 passed (8.9s)`。停止正式 API/Worker/前端与 RAGFlow 后创建并验证加密
  归档，随后先删除源 MySQL 容器、RAGFlow 容器及四个源 Volume，再恢复到全新 MySQL 和四个全新
  Volume；恢复页面 `1 passed (2.5s)`，验证真实数据集/文档、员工绑定、1 条外部引用、审计哈希链
  且浏览器未直连 RAGFlow。端到端恢复 63 秒，低于 120 分钟 RTO
- GREEN：归档单元测试 `7 passed`，目标模块行覆盖率 81%；最终后端权威覆盖率全量 `624 passed,
  13 skipped in 207.58s`，总体行 91.18%、分支 72.41%、核心行 93.28%、核心分支 75.00%，全部
  超过 H7-02 冻结门禁。Ruff、290 文件格式、严格 Mypy 274 个源/测试文件、uv lock、85 包安全
  审计、Alembic `0016` head/无模型漂移、OpenAPI/事件/生成 DTO、架构、CI/覆盖率/dev/real、
  平台/RAGFlow/备份、ShellCheck、Secret 与 bundle 门禁通过。前端 23 files/107 tests，行 87.35%、
  分支 76.04%，ESLint、TypeScript、生产 Build、零高危依赖审计通过；标准 Demo 两轮会话与持久
  Worker 恢复 E2E 最终 `1 passed (8.5s)`
- 清理与文档：隔离演练的源/恢复 MySQL、RAGFlow 容器、网络、四组源/恢复 Volume、临时归档、
  密钥、Token、恢复目录、页面产物和测试业务数据已精确删除；最终停止 API、Worker、前端、平台
  MySQL、RAGFlow、浏览器与项目专属 Colima，18200、18280、19380、19506 和演练端口无监听。
  正式稳定 Volume、官方镜像、冻结依赖、既有 0600 Token 和用户授权百炼 Demo Key 保留；根/infra
  README、工程结构、后端架构、脚本说明、CI 和本路线图已同步，未修改 RAGFlow submodule/源码/
  镜像/容器内文件，也未执行远程部署
- 遗留：无；下一任务 S10-07 在隔离环境建立生产构建、部署与回滚链路；实际创建远程资源和首次
  发布必须再次取得用户明确部署指令
...
- 日期：2026-07-21
- 提交：本任务提交（见 Git 历史）
- RED：先后建立生产运行配置、镜像/Compose/发布管理、安装 wheel 迁移、RAGFlow 私有 CA、CI
  复用、RAGFlow Edge 就绪、跨节点覆盖层、远程公开验收地址与私钥分离契约。首次分别因缺少
  `RuntimeEnvironmentSettings`、`infra/production/manage.sh`、wheel 中错误迁移路径、适配器不接受
  CA bundle、CI 未调用生产门禁、发布未等待 RAGFlow TLS Edge、基础 RAGFlow 编排依赖业务节点
  Docker network、远程验证固定解析到 `127.0.0.1`，以及业务节点无条件要求 RAGFlow 私钥而失败，
  均在保留目标失败证据后进入实现
- 镜像与运行时：后端固定 Python 3.12.11/uv 0.11.16、前端固定 Node 22.17.1/Nginx 1.28.0，
  所有基础镜像和 MySQL 8.4.10 均锁定 digest；生产层使用数值非 root、只读根文件系统、
  `no-new-privileges`、cap drop、受限 tmpfs/CPU/内存和健康检查，构建上下文排除 Git、环境文件与
  生成物。运行配置关闭本地 loopback 假设，强制 HTTPS CORS、Secure Cookie、非 loopback 数据库
  与 RAGFlow 地址，并显式加载私有 CA；httpx 继续不信任环境代理，但不再因此绕过指定 CA
- 双节点与发布：默认 `external` 拓扑把业务 Edge/Web/API/Worker/MySQL 与 RAGFlow 官方栈分开；
  RAGFlow 节点仅向私网发布 TLS 9443，业务节点只持有信任 bundle，RAGFlow 服务端私钥不跨节点。
  MySQL 只在 internal 网络，API/Worker 另接受防火墙约束的 egress 网络访问私网 RAGFlow 与百炼；
  `local-shared-network` 只作为 drill 覆盖层。发布清单把 Git revision、时间戳和本地镜像 ID 固定为
  `sha256`；非本机 Docker context 需要精确确认词，公开验证可配置真实 HTTPS URL且不再强制解析
  本机。流程固定为 preflight→MySQL 就绪→显式 `alembic upgrade head`→候选 API/Web→Edge 切流
  与 real/RAGFlow 验证→候选 Worker→旧槽停止；回滚恢复上一代码/流量但绝不自动降级 schema
- 生产同路径与失败矩阵：专属 32 GiB Colima 从真实 RAGFlow v0.25.6 启动，构建两份不可变
  release，将空生产 MySQL 从 Alembic base 迁移到 `20260721_0016`，先由蓝槽提供 HTTPS，再由正式
  Chromium 验证五入口首次加载与模块复用 `2 passed (6.7s)`。第二份 release 切到绿槽并停止旧槽；
  随后停止 active API 注入故障，公开入口按预期失败，脚本恢复蓝槽 API/Web/Worker、重新切流并
  验证成功，数据库 schema 保持向前。构建中真实暴露并修复 pnpm workspace 缺失、wheel 迁移路径、
  Nginx 只读临时目录、internal network 发布端口、私有 CA 与 `trust_env=false`、RAGFlow Edge 就绪
  竞态和浏览器 host resolver 冲突，没有跳过任何阶段
- GREEN：后端权威覆盖率全量 `633 passed, 13 skipped in 205.22s`，总体行 91.16%、分支 72.62%、
  核心行 93.28%、核心分支 75.00%，全部超过冻结门禁；Ruff、292 文件格式、严格 Mypy 276 个
  源/测试文件、uv lock 与 85 包依赖审计通过。前端 23 files/107 tests，行 87.35%、分支 76.04%，
  ESLint、TypeScript、生产 Build、五路由 bundle、生成契约与零高危依赖审计通过。平台/RAGFlow/
  备份/生产、dev/real/CI/覆盖率/bundle/Secret 契约、ShellCheck、Bash 语法和 `git diff --check`
  通过；完整 drill 最终输出“本地双节点生产同路径演练通过”
- 资源、清理与文档：4C4G 不满足当前业务节点蓝绿 API/Worker/MySQL 的生产余量，也低于 RAGFlow
  最低线；Runbook 固定业务节点 4C16G 最低/8C16G 推荐、RAGFlow 4C16G 起步/8C32G 推荐，并要求
  S10-08 以目标负载复核。演练 trap 已删除生产容器、网络、MySQL Volume、临时证书/凭据和状态目录，
  按需启动的 RAGFlow 已停止；稳定 Volume、固定镜像、0600 Token 与用户授权 Demo Key 保留。根/
  infra README、生产 Runbook、CI 和本路线图已同步，RAGFlow submodule/源码/官方镜像未修改，也未
  访问或部署任何远程资源
- 遗留：没有 S10-07 范围内遗留；下一任务 S10-08 完成容器/SAST/权限与输入攻击、并发/容量/
  长连接/故障压测、SLO/告警和新租户全链路最终验收。实际采购规格确定、云资源创建和首次发布仍
  必须取得用户明确指令

### S10-07A RAGFlow v0.26.4 维护升级

- 状态：✅ 已完成
- 日期：2026-07-21
- 提交：本任务提交（见 Git 历史）
- RED 与真实升级阻塞：版本一致性契约首次按预期报错“RAGFlow 版本未固定为 v0.26.4”，模型
  配置测试首次因新实例常量尚不存在而收集失败；全新 v0.26.4 栈随后真实暴露 Python Provider API
  注册 `Tongyi-Qianwen` 时遍历静态目录并因不支持的 OCR 模型返回 code 102。Docker Hub 拉取官方
  镜像又以 EOF 失败，改用官方文档给出的 SWR 镜像源后成功。首次资源验收还证明未暂存的 gitlink
  会被 `git submodule update` 恢复到父仓库旧提交；第二次则因浏览器缺少正式 API/认证变量关闭
  失败。以上失败均保留目标原因后修复，没有跳过版本、供应商、浏览器或 30 分钟门禁
- 版本、备份与迁移：父仓库 gitlink、`VERSION`、`UPSTREAM_COMMIT`、活动配置和镜像统一固定
  官方 tag `v0.26.4`/commit `cb93883f3f8c975eecb2fed81210effeb3bdb06f`；本机 amd64 镜像 ID 为
  `sha256:e0048bb5ee60f8bcd2e9a2c4851de80f39a0b7318ad4e55bf7bbcef126eaa9ac`。升级前创建并验证
  `.local/backups/common-agent-20260721T114312Z.cab` AES-256-GCM 认证冷备，密钥保持独立 0600
  文件且不写入台账；全程保留四个原生 Volume，未执行 `down -v`。官方启动迁移把 RAGFlow 表数
  从 32 增至 42，并完成 2 条旧 tenant model 记录到 1 个 provider 及 v0.26.1 marker 的迁移；升级
  前后正式库存均为 `datasets=0, documents=0`，没有把空库存冒充用户数据恢复证据
- 百炼公开 API 适配：不修改 RAGFlow submodule、源码、镜像内文件、安装包或数据库；只经
  v0.26.4 公开 Provider/Model API 建立 `common-agent-embedding` 与 `common-agent-rerank` 两个
  `OpenAI-API-Compatible` 实例，分别使用百炼官方 `compatible-mode/v1/embeddings` 和
  `compatible-api/v1/reranks`。知识库与检索固定
  `text-embedding-v4@common-agent-embedding@OpenAI-API-Compatible` 和
  `qwen3-rerank@common-agent-rerank@OpenAI-API-Compatible`；这只是同一阿里百炼供应商的传输
  适配，不是模型网关或多供应商兜底。fresh/main 两套配置均报告 embedding/rerank/defaults ready，
  各自真实适配器生命周期与检索 `2 passed`
- 恢复、生产与业务链：独立 v0.26.4 源环境经正式页面创建并解析真实文档，认证归档后销毁源
  MySQL/RAGFlow 四卷并恢复到空环境，源/恢复浏览器各 `1 passed`，58 秒恢复并验证 1 条外部引用；
  生产同路径重新构建两份不可变 release，真实 RAGFlow、TLS Edge、空库迁移、Chromium 五入口、
  蓝绿切流、active API 故障与代码回滚全部通过，入口套件 `2 passed (5.7s)`。最终 real 验收使用
  隔离 `common_agent_test` 和受控认证状态，真实 RAGFlow 生命周期 `1 passed (17.31s)`，从空业务
  数据完成知识库→员工→两轮带引用会话→工作流手动/员工触发及刷新恢复 `1 passed (25.1s)`；
  冷启动 38 个样本在 89.91 秒 ready，VM/容器峰值 6.85/6.81 GiB；随后 180 个连续样本覆盖完整
  1800 秒窗口，VM/容器峰值 7.28/7.23 GiB，全部 ready，采样错误、Swap、持续 Swap 增长、容器
  重启和 OOM 均为 0
- GREEN：后端权威覆盖率全量 `633 passed, 13 skipped in 206.85s`，总体行 91.17%、分支 72.52%、
  核心行 93.28%、核心分支 75.00%；本次最终模型/配置定向 104 passed，Ruff 与严格 Mypy 通过。
  前端 23 files/107 tests，行 87.35%、分支 76.04%，本次系统状态定向 3 files/18 tests；ESLint、
  TypeScript、生产 Build、五路由 bundle、OpenAPI/事件/生成 DTO、uv/pnpm 冻结锁与两类依赖审计
  均通过。平台/RAGFlow/备份/生产、dev/real/CI/覆盖率/bundle/Secret、ShellCheck、Bash 语法和
  `git diff --check` 门禁通过
- 资源、清理与远程边界：独立 fresh、恢复、生产蓝绿与失败 soak 的容器、网络、Volume、临时
  凭据/证书、业务测试数据、认证状态和进程均已精确清理；最终 real trap 停止平台 MySQL、RAGFlow
  五个容器和项目专属 Colima，18200、18280、19380、19381、19506 无监听。正式稳定 Volume、
  v0.26.4 镜像、0600 Token、用户授权 Demo Key 与升级前认证备份保留。4C4G 仍不满足业务蓝绿和
  RAGFlow 官方最低线，采购 Runbook 继续建议业务节点 8C16G、RAGFlow 节点 8C32G，S10-08 再按
  目标并发复核；本任务没有采购、创建、连接或发布任何远程资源
- 遗留：没有 S10-07A 范围内遗留；下一任务仍为 S10-08。实际采购规格确定、云资源创建和首次
  发布必须取得用户明确指令

### S10-07B Review 生产一致性修复

- 状态：✅ 已完成
- 日期：2026-07-21
- 提交：本任务提交（见 Git 历史）：`772bf45`、`71d66dd`、`a6dbc26`、`3c85821`、`dbf8a75`、
  `8df52fc`、`880824c`、`b86d99f`、`70bfe7e`、`f0bcf08`、`5feca03`、`273479f`、`8aefdbf`、
  `0841b4c`、`33b4db1`
- RED 与审查结论：分别用反向代理伪造来源、审计仓储失败、RAGFlow 101+ 数据集、两个 API 实例
  并发修改同一资源、两个 Worker 等待同一任务执行锁及租约丢失建立目标失败；现状依次暴露任意
  `X-Forwarded-For` 可拆分限流、业务成功后才写审计、只读取 RAGFlow 前 100 条、跨进程仅有本地锁、
  Worker 在过期租约接管后仍可能执行副作用。全量门禁又真实捕获“停止请求已返回 202，晚到完成仍
  写成 completed”的工作流竞态，以及并发回归断言的严格 Mypy 类型错误；均保留失败后修复，没有
  通过重跑、放宽断言或 Mock 结果掩盖
- 实现收口：生产运行时只信任明确配置的 Edge 代理并由 Edge 覆盖转发来源；写操作先持久化
  `attempted` 审计意图，再执行服务并把最终 outcome 追加到同一哈希链，审计不可用时关闭失败且业务
  无副作用，Alembic `20260721_0017` 同步约束新 outcome。资源引用修改使用 MySQL 命名锁与进程内锁
  协同；Worker 在任务级共享锁内重新确认租约和停止状态，租约丢失立即栅栏业务处理。知识适配器按
  RAGFlow 正式分页遍历租户全部数据集，再完成平台权限过滤、搜索和游标分页，不以供应商默认 100 条
  当作全量。持久工作流停止意图成功后立即把 MySQL 权威摘要收敛为 stopped，晚到 Worker 完成只能
  观察终态；Playwright 固定 `chromium-headless-shell`、`headless` 与无启动窗口参数
- 生产同路径：本地双节点生产演练从空 MySQL 迁移至 `20260721_0017`，经正式 TLS Edge 让前五次
  不同伪造 `X-Forwarded-For` 登录均返回 401、第六次返回 429，证明限流按 Edge 看到的真实客户端
  生效；同一演练由无窗口 Chromium 完成五入口 `2 passed (5.3s)`，再执行蓝→绿切流、活动 API 故障
  注入和代码回滚且 schema 保持向前，最终输出“本地双节点生产同路径演练通过”。审计故障经正式
  Uvicorn/MySQL HTTP 写入口返回 503 且业务记录不存在；两个独立 Uvicorn 进程经正式 PUT 在第三连接
  持有同一 MySQL 锁时均保持等待，释放后两次返回 200，分布式锁集成共 `2 passed`。正式 API→独立
  Worker→MySQL 路径覆盖正常执行/停止/重试，租约丢失与等待锁栅栏由故障注入精确定位
- RAGFlow 与页面验收：正式 `v0.26.4` 栈恢复百炼 embedding/rerank/defaults 后，通过平台 API 创建
  105 个唯一知识库；无窗口 Chromium 从正式 React 知识库页面搜索、按 20 条连续加载至 105 条并
  看到末项，`1 passed (8.3s)`，finally 再经正式 API 删除，RAGFlow 残留为 0。浏览器调试日志确认
  实际二进制为 `chromium_headless_shell-1228/.../chrome-headless-shell`，退出后进程与临时目录关闭
- GREEN：权威全量共收集 657 项，后端 `644 passed, 13 skipped in 158.36s`；13 项仅为未在普通
  全量中显式启用的既有真实 RAGFlow/百炼/Deep Agents 门禁，本任务所需真实链路已单独执行。总体
  行 91.19%、分支 73.07%、核心行 93.42%、核心分支 75.43%，均超过冻结阈值；Ruff、296 文件格式、
  严格 Mypy 279 个源/测试/门禁文件、uv lock 和 85 包审计通过。前端 23 files/109 tests，行
  87.24%、分支 75.91%，冻结安装、ESLint、TypeScript、生产 Build、零高危审计、OpenAPI/双事件/
  生成 DTO 漂移及五路由 bundle 预算通过；平台/RAGFlow/备份/生产、dev/real/CI/覆盖率/E2E/Secret
  契约、Bash 语法、ShellCheck 和 `git diff --check` 全部通过
- 清理与边界：105 个验收知识库及关联数据已删除，四个临时生产候选镜像、生产容器/网络/Volume、
  失败验收产物、API/Worker/Vite/Playwright 进程均已精确清理；旧 `infiniflow/ragflow:v0.25.6`
  镜像及其遗留 `tei-cpu` 容器/本地 embedding 镜像已删除。18200、18280、18443、19380、19381、
  19506 无监听，平台与 RAGFlow 容器已停止，项目专属 Colima 已停止；稳定原生 Volume、v0.26.4
  固定镜像、构建缓存、0600 Token、现有 Demo Key 与升级前认证备份保留。RAGFlow submodule、源码、
  v0.26.4 镜像和容器内文件均未修改，也未访问、采购或部署远程资源
- 遗留：无；下一任务 S10-08 执行生产输入攻击/权限/SAST、目标并发与容量、长连接/故障压测、
  SLO/告警和新租户全链路最终验收
...
- 日期：2026-07-22
- 提交：本任务提交（见 Git 历史）
- RED：先把凭据领域、OpenAPI 契约和注册/恢复/成员创建表单改为要求正好 8 位密码可通过；旧实现
  仍要求至少 15 位，后端 2 个目标测试与前端 3 个用户表单测试按预期失败，没有放宽断言或跳过
- 实现：首位所有者注册、恢复码重置和租户成员初始密码统一为 8 至 128 位；领域常量、FastAPI
  请求模型、OpenAPI 产物和 React 表单提示保持一致。登录接口继续只校验密码非空，把真实密码策略
  留给凭据校验，避免旧账号因请求层长度约束无法登录
- 生产同路径：正式 Uvicorn→认证/租户 HTTP 入口→独立 MySQL 链路分别使用 8 位 `Owner#28`、
  `Reset#28` 和 `Viewer#8` 完成所有者注册、恢复重置与成员创建；运行中的正式开发 API OpenAPI
  同时确认三处 `minLength` 均为 8，站点仍处于可由用户自行注册首位所有者的状态，未代替用户创建
  或保留测试账号
- GREEN：权威全量共收集 658 项，后端 `645 passed, 13 skipped in 166.57s`；13 项仅为普通全量
  未显式启用的既有外部 RAGFlow/百炼/Deep Agents 门禁，本任务正式 Uvicorn/MySQL 验收未跳过。
  总体行 91.22%、分支 73.15%、核心行 93.28%、核心分支 74.86%；严格 Mypy 278 个源/测试文件、
  Ruff 与格式检查通过。前端 23 files/109 tests，行 87.24%、分支 75.91%、函数 87.58%、语句
  85.77%；ESLint、TypeScript、生产 Build、五路由 bundle 预算、OpenAPI/生成 DTO 漂移、Secret
  治理和 `git diff --check` 全部通过，全程未启动有头浏览器
- 清理与边界：正式 HTTP 验收使用独立测试库并完成回收，没有留下测试认证数据、浏览器进程、临时
  容器或镜像；当前供用户查看的本机开发服务不属于临时测试资源。没有访问、采购或发布远程资源
- 遗留：无；用户后续新增 S10-07D 至 S10-07J，下一任务改为 S10-07D 知识库人工验收样本文档；
  产品增补全部完成后再进入 S10-08
...
- 日期：2026-07-22
- 提交：本任务提交（见 Git 历史）
- 交付物：`test-data/knowledge-base/corpus/` 提供 12 份可独立上传的虚构业务文档，覆盖 Markdown、
  TXT、5 页 PDF 和 4 页 DOCX；`expected-answers.md` 单独保存且醒目标注禁止上传，包含 24 道精确
  事实、语义改写、相近指标、版本冲突、临时例外、跨文档多跳、专题检索与无答案问题。生成脚本
  可重复构建两个二进制文件，README 给出真实验收、评分和清理路径
- 内容质量：真实解析器共提取 20,139 个正文字符，最短单篇 1,436 字；12 份文档分别包含唯一
  标记，全部能从各自文件提取命中。内容围绕产品、运维、安全、试点验收、事故复盘、治理、模型
  评测、知识编写、工作流、容量成本、区域推广和隐私处理展开，数字、日期、旧值、新值及例外均有
  可区分上下文，没有使用重复段落或无意义文本凑量
- 真实交付物验收：重新执行正式生成命令后，PyPDF 与 python-docx 从交付的 PDF/DOCX 成功提取
  正文和唯一标记，`file` 确认为 5 页 PDF 1.4 与 Microsoft OOXML；PDF 5 页和 DOCX 4 页全部在
  后台渲染为 150 DPI 图片并逐页人工检查，中文字体、标题、表格、页码和正文无缺字、裁切、重叠
  或越界。该任务交付的是后续真实 RAGFlow 召回验收物料，不提前把标准答案或语料写入用户知识库；
  正式页面批量上传、RAGFlow 解析与 24 题召回验证归入依赖这些物料的 S10-07J
- GREEN 与清理：生成脚本 Ruff 检查和格式检查通过，Secret 扫描器自校验及治理扫描通过，
  `git diff --check` 通过，所有文件均小于 20 MiB；所有名称、人员、组织、指标与编号均为虚构，
  没有真实凭据或用户数据。无窗口渲染产物和临时转换文件已在验收后精确删除，不留下浏览器进程、
  容器、镜像或外部测试数据
- 遗留：无；用户随后提供设计规范与交互原型，下一任务改为 S10-07K 设计基线与项目 UI 规则接入

### S10-07K 设计基线与项目 UI 规则接入

- 状态：✅ 已完成
- 日期：2026-07-22
- 提交：本任务提交（见 Git 历史）
- 资料与边界：用户提供的 `DESIGN.md` 和 533,989 字节自包含交互原型原样复制到
  `docs/design/`，SHA-256 分别为 `5d203a5525bc0b0654e9778cbdec29a686e7b8e24f69d1b1df2d80c240cc953c`
  与 `7420480bdea061f312c45f64e99dd415d4d592d4a9648d72863c753d37253772`，均与下载目录原件一致；
  README 明确设计规范、交互原型和项目产品/数据事实源之间的职责
- 项目规则：根 `CLAUDE.md` 已把 `docs/design/DESIGN.md` 加入每次开发必读基线，并强制全部新增
  与改造 UI 通过统一 Token 和公共组件遵守规范；HTML 只能参考布局、交互和代码，不得直接替代
  React 或定义路由、权限、模型目录与数据契约。允许复用负荷曲线和蓝色火花节点动效 Logo，但
  明确禁止照搬 `PowerAI` 名称、电力行业定位、示例账号、供应商目录与演示数据
- 真实资料验收：使用 agent-browser 0.27.1 默认无头 Chromium 从仓库 `file://` 路径离线打开 HTML，
  先经过原型登录，再依次进入对话、数字员工、模型管理、知识库、工作流列表和工作流画布；各页面
  标题、导航、表单/按钮、卡片/表格和画布节点均可访问。截图仅用于本轮逐页人工比对，未把原型
  的固定回答、假模型或电力行业数据当成正式功能证据
- GREEN 与清理：来源哈希、规则引用、20 MiB 上限、Secret 治理和 `git diff --check` 全部通过；
  浏览器会话已按会话 ID 关闭，6 张临时截图精确删除，未留下 Chrome/Chromium 进程、服务、容器、
  镜像或外部数据
- 规划补充：S10-07L 负责现有正式页面统一设计改造；所有产品增补任务完成后，S10-07M 单独执行
  全仓架构、性能和代码结构 Review 与逐项整改，S10-08 依赖该 Review 通过后再开始
- 遗留：无；下一任务 S10-07L 现有页面统一设计改造
...
- 日期：2026-07-22
- 提交：本任务提交（见 Git 历史）
- RED：先建立集中主题、动效品牌标识和主导航语义契约；目标测试首次因 `designSystem`、
  `BrandLogo` 与“主导航”均不存在而按预期失败。实现后没有用快照放宽视觉要求，继续以正式 DOM、
  计算样式、响应式宽度和用户可见文案建立回归契约
- 实现：新增 Ant Design 集中主题与 CSS Token，以 `#191919` 黑色主操作、`#FAFAFA` 暖白背景、
  细边框、低阴影和功能状态色统一认证、AI 会话、数字员工、知识库、工作流与审计页面；App Shell
  统一使用 Lucide 图标、语义化主导航和 232/72px 响应式侧栏。复用原型的负荷曲线与蓝色火花动效
  构建 Common Agent Logo，没有引入 PowerAI 名称、电力行业数据、原型假供应商或新业务路由；
  React、Ant Design、权限、API、表单、键盘和业务状态语义保持不变
- 生产同路径与视觉验收：E2E 入口支持显式隔离 API/前端端口并把同一端口贯穿 Uvicorn、CORS、
  Vite 构建和 Playwright；使用独立 `18201/18281`、正式 Uvicorn、MySQL 测试库、独立 Worker、
  `pnpm build + vite preview` 与 `chromium-headless-shell` 完成设计/加载套件 `4 passed (10.5s)`。
  随后经正式页面创建数字员工、进入审计页面、按资源过滤并经正式 API 清理，`1 passed (2.6s)`；
  agent-browser 0.27.1 默认无头会话以 1440×1000 逐页检查认证、会话、员工、知识库、工作流、审计，
  并在 760×900 检查员工和工作流，文档宽度均为 760、无横向溢出、无页面脚本错误或 PowerAI 文案
- 资源故障与修复：首轮隔离验收暴露旧入口会为 12 GiB 轻量套件把已足够的高配 Colima 降档，
  进而停止稳定容器；当场中止并恢复平台 MySQL、RAGFlow 与用户开发服务，入口改为现有资源
  `>=` 目标预算即复用，并由 Shell 契约锁定。另两次失败分别保留了 API 未实际绑定隔离端口、
  生产构建未注入隔离 API 的证据，均修复后重跑同一路径通过，没有改用 Mock 或开发服务器冒充
- GREEN：前端 `24 files / 111 tests`，语句 85.84%、分支 76.00%、函数 87.64%、行 87.30%；
  ESLint、TypeScript、生产 Build、五路由首次 JS 图与 500KB 单 chunk 预算、OpenAPI/生成 DTO 漂移、
  pnpm 冻结安装、零高危依赖审计、Secret 治理、E2E/ShellCheck 契约和 `git diff --check` 全部通过
- 清理与边界：本轮 `18201/18281` API/Worker/前端、Chromium/Playwright/agent-browser 进程、
  测试账号/工作区、截图、Trace 和失败验收目录已精确清理；供用户联调的 `18200/18280`、平台
  MySQL 与 RAGFlow v0.26.4 已恢复并继续运行，未重复构建业务镜像、未删除稳定 Volume 或固定镜像，
  也未访问、采购或发布远程资源
- 遗留：无；下一任务 S10-07E 用户模型管理
...
- 日期：2026-07-22
- 提交：本任务提交（见 Git 历史）
- RED：先建立模型配置领域、正式 Uvicorn→MySQL HTTP、租户隔离、Demo 指定模型验证、引用阻断、
  React 页面和 App 导航测试；领域模块、API 路由、前端 API/页面均不存在时分别以模块缺失和 404
  按预期失败。Viewer 全写入口测试随后先证明“测试调用”按钮仍可操作，再改为只读；模型服务分页
  测试还复现了启用筛选超过一页时冒号作用域不符合游标协议的问题，未通过放宽游标校验解决
- 实现：新增 `ModelConfiguration` 领域、Alembic `20260722_0018`、租户内名称/百炼标识唯一约束、
  稳定 keyset 分页和 `model_configuration_references` 通用引用表。正式 API 支持列表、创建、详情、
  编辑、删除和测试调用，所有写入均经租户资源锁、MySQL 事务和固定元数据审计；删除先查引用并由
  复合外键兜底。首版只允许 `bailian`，前端不接收 API Key/Base URL，也不加载百炼完整模型目录
- 页面：左侧新增“模型管理”，按统一设计规范提供搜索、卡片、创建/编辑、启停、测试调用和受控
  删除；仅展示用户已创建配置，停用状态清晰可见，Viewer 的 POST/PUT/DELETE 入口全部禁用。六个
  页面保持独立动态入口，模型页首次 JS 图 1,463,048 bytes，最大共享 chunk 189,644 bytes，均低于
  固定预算
- 生产同路径：使用隔离 `18420/18480`、正式 Uvicorn、项目 MySQL、独立 Worker、`pnpm build +
  vite preview` 生产资产和
  `chromium-headless-shell` 从页面创建 `qwen-plus` 配置，经平台服务端统一凭据和正式百炼适配器
  真实返回成功；刷新后恢复配置，编辑名称并停用后再次刷新仍保持。测试夹具只负责建立/解除真实
  MySQL 引用前置条件，用户删除动作始终从正式页面发起：有引用时收到 409 和解除指引，解除后返回
  204 并从页面消失；最终复验 `1 passed (6.0s)`，未用 Mock、进程内客户端或日志代替完成验收
- 失败闭环：首轮全量门禁发现数据库迁移头断言仍为 `0017`，迁移恢复用例因此把测试库版本标记
  回旧值并让后续测试关闭失败；修正两个权威迁移头断言并把测试库恢复为已存在表对应的 `0018` 后，
  原失败组 10 项和全量均通过。前端首轮覆盖率 85.46% 低于 86.17% 门槛，补齐正式 API/Zod 契约
  测试而未降低阈值；六路由 Bundle 夹具也随新增入口同步并继续验证缺路由关闭失败
- GREEN：后端全量 `668 passed, 13 skipped in 167.80s`；13 项仅为普通全量未显式启用的既有外部
  RAGFlow/百炼/Deep Agents 门禁，本任务真实百炼路径已由独立生产同路径验收通过。总体行 91.11%、
  分支 72.85%、核心行 93.48%、核心分支 75.96%；模型领域自身行/分支 100%，模型服务 96%。Ruff、
  309 文件格式、严格 Mypy 290 个源/测试文件、uv lock 和 85 包审计通过。前端 26 files/119 tests，
  行 86.77%、分支 76.20%、函数 86.89%、语句 85.44%；ESLint、TypeScript、生产 Build、冻结安装、
  零高危审计、OpenAPI/生成 DTO 漂移、六路由 Bundle 预算全部通过
- 工程门禁与清理：CI/覆盖率/Bundle/E2E 资源档位、Secret、平台 MySQL、RAGFlow v0.26.4、备份、
  生产构建/回滚契约、ShellCheck 和 `git diff --check` 均通过。验收 API/Worker/Vite、无头 Chromium、
  Trace/截图和模型配置/引用/认证测试数据已精确清理；稳定开发服务、平台 MySQL、RAGFlow 固定镜像
  与 Volume 保留复用，没有新增重复业务镜像，也未访问、采购或发布远程资源
- 遗留：无；下一任务 S10-07F 数字员工默认模型

### S10-07F 数字员工默认模型

- 状态：✅ 已完成
- 日期：2026-07-22
- 提交：本任务提交（见 Git 历史）；全量门禁发现并独立修复的工作流终态竞态为 `a940df2`
- RED：先让数字员工领域、正式 API/OpenAPI、React 表单与运行时请求必须携带模型配置。旧实现分别
  以 `EmployeeConfiguration`/`Employee` 缺少字段、正式 API 返回 422、前端 Zod 拒绝响应且页面没有
  模型选择器失败；迁移回滚首轮还真实暴露 MySQL 外键依赖索引必须先删约束，修正顺序后在隔离
  MySQL 完成 `0019 → 0018 → 0019`。没有用可选字段或进程默认模型维持旧假路径
- 实现：Alembic `20260722_0019` 为已有租户创建或复用 `qwen-plus` 平台默认模型，回填所有现有/预置
  员工并建立租户复合外键、索引和通用模型引用；新建/编辑只接受当前租户模型配置，停用配置只能
  原样保留已有绑定，不能新选，删除由引用表和员工复合外键双重关闭失败。员工查询联表一次解析
  模型标识，不重复持久化供应商标识；Deep Agents 运行时按每轮员工模型标识解析并缓存百炼适配器，
  API 与独立 Worker 共用相同正式装配和关闭生命周期
- 页面与协议：数字员工卡片显示默认模型和停用状态，创建/编辑选择器只提供模型管理中的可用配置，
  当前停用绑定在编辑时可见但不可重新选择；模型列表失败时保存入口关闭并可重试。OpenAPI、生成 DTO、
  API Zod、所有既有员工 E2E 夹具和多租户页面链同步到强制模型字段；迁移后正常开发库中员工空绑定、
  跨租户失效绑定和缺失引用均为 0
- 生产同路径：隔离 `18201/18281` 使用正式 Uvicorn、MySQL、独立 Worker、生产 Build/Preview、真实
  RAGFlow v0.26.4、Deep Agents 和百炼，无头 Chromium 从模型页创建 `qwen-turbo`，从员工页以平台
  默认模型创建后刷新，再编辑切换并刷新；停用后验证保留已有绑定、新建不可选和删除返回 409，最后
  从员工进入会话获得真实回复。验收进程故意把 `BAILIAN_MODEL` 设为不存在值，仍 `1 passed (10.4s)`，
  证明回复使用员工持久化的 `qwen-turbo` 而非进程回退；平台员工/真实 RAGFlow 回归 `2 passed (54.2s)`
- 失败闭环：首轮全量发现旧模型 CRUD 用例与新增平台 `qwen-plus` 唯一配置冲突，以及运行时契约夹具
  未提供强制模型字段；改用独立测试模型并补齐模型格式断言后原失败组 `31 passed`。第二轮全量又
  捕获正式停止入口返回 202 后 Worker 晚到完成覆盖 stopped 的既有跨进程竞态；新增 MySQL 回归先
  复现晚到更新成功，再用运行态条件更新固化首个终态，正式 HTTP 连续 5 次通过并按单问题提交
  `a940df2`，最终全量没有靠重跑掩盖失败
- GREEN：后端权威覆盖率全量 `676 passed, 13 skipped in 171.71s`；13 项为普通全量未显式启用的
  既有外部门禁，本任务真实百炼链已单独执行。总体行 91.02%、分支 72.67%、核心行 93.20%、核心
  分支 75.65%；Ruff、格式、严格 Mypy、uv lock 和 85 包审计通过。前端 `26 files / 120 tests`，
  行 86.68%、分支 75.69%、函数 86.74%、语句 85.33%；ESLint、TypeScript、生产 Build、六路由
  Bundle、冻结安装、零高危审计和 OpenAPI/生成 DTO 漂移全部通过
- 工程门禁与清理：CI/覆盖率/Bundle/E2E 资源档位、Secret、平台 MySQL、RAGFlow、备份、生产构建/
  回滚、ShellCheck、Bash 语法和 `git diff --check` 通过；正常开发 API/Worker 已用新代码重启，数据库
  位于 `0019` 且 real/RAGFlow 状态健康。隔离 API/Worker/前端、无头 Chromium、Trace/截图、模型/
  员工/会话/认证测试数据和临时端口均已精确清理；稳定容器、Volume、固定镜像和前端服务保留复用
- 遗留：无；下一任务 S10-07G 通用会话、自动建会话与逐轮模型切换

### S10-07G 通用会话、自动建会话与逐轮模型切换

- 状态：✅ 已完成
- 日期：2026-07-22
- 提交：本任务提交（见 Git 历史）
- RED：先把会话来源、通用会话当前模型和助手实际调用模型设为强约束；旧领域对象不接受这些字段，
  正式 `/api/v1/conversation-turns` 返回 404，`/chat` 仍要求员工和预建会话，前端也没有模型选择器。
  MySQL HTTP 回归进一步锁定未知/停用/跨租户模型不得留下空会话，员工临时切模不得改员工默认值，
  首轮幂等重放仍只能有一组用户/助手消息与一个确定性任务
- 后端与迁移：Alembic `20260722_0020` 增加 `generic | employee` 来源、通用会话当前模型、助手本轮
  模型配置/百炼标识快照及复合外键/索引，并为既有员工会话和助手消息回填不可漂移事实。
  `ConversationExecutionTargetResolver` 统一解析通用/员工目标，`ConversationTurnCoordinator` 让
  首次发送在同一 MySQL Unit of Work 原子创建会话、两条消息和持久任务；独立 Worker 只使用任务
  对应助手消息的模型快照，通用对话不访问 RAGFlow，员工对话继续复用正式知识检索和 Deep Agents
- 页面与协议：点击 AI 会话直接得到可输入的空白通用对话，不调用预建会话接口；首次发送成功后
  才写入 URL 和历史快照。输入区下方只展示当前工作区用户已创建且启用的模型，支持逐轮切换和
  有界分页；通用会话保存当前选择，员工会话在页面内临时切换但不修改员工。模型目录加载失败时
  关闭发送并提供正式重试，没有启用模型时明确引导去模型管理。OpenAPI、事件 Schema、生成 DTO、
  Zod 和既有聊天 E2E 已同步到严格来源/模型字段
- 生产同路径：隔离 `18201/18281` 使用正式 Uvicorn、项目 MySQL、独立 Worker、生产 Build/Preview、
  Deep Agents 与真实百炼，无头 `chromium-headless-shell` 从模型管理页创建 `qwen-turbo`，直接进入
  空白通用 AI 首次发送并得到真实标记，刷新恢复会话与模型，再切回平台 `qwen-plus` 完成第二轮真实
  回复，最后从正式页面删除会话和模型，复验 `1 passed (7.6s)`。员工默认模型回归 `1 passed
  (10.9s)`，真实员工/RAGFlow 平台回归 `2 passed (48.5s)`，证明新增通用路径没有绕过或破坏员工
  知识链；三组都通过生产原入口，不以 Mock、进程内客户端或日志断言代替完成验收
- 失败闭环：通用 E2E 首轮只因页面正式文案与断言相差“输入/发送”失败，截图确认产品状态正确后
  修正精确断言；第二轮已完成第一条真实百炼回复，只因同一模型名称同时出现在选择器和删除按钮
  触发严格定位歧义，改为角色限定后通过。权威覆盖率首次发现新增领域分支把后端核心行降到
  `92.67%`、前端分支降到 `74.86%`，均未降低门槛：补齐非法来源/模型快照/时间倒退和模型加载
  失败/无可用模型状态后恢复。最终严格 Mypy 还发现测试替身仍使用旧员工专用知识协议，修正为
  通用知识主体并以定向 `12 passed` 证明装配一致
- GREEN：后端权威覆盖率全量 `690 passed, 13 skipped in 191.49s`；总体行 `91.08%`、分支
  `72.73%`、核心行 `93.38%`、核心分支 `76.75%`，六项均高于固定门槛。Ruff、320 文件格式、
  严格 Mypy 299 个源/测试文件、uv lock 与 85 包安全审计通过。前端 `26 files / 124 tests`，行
  `86.55%`、分支 `75.29%`、函数 `86.32%`、语句 `85.23%`；ESLint、TypeScript、生产 Build、
  冻结安装和零高危审计通过，`/chat` 首次 JS 图 `1,486,002 bytes` 低于固定 1.5 MB 预算
- 工程门禁与清理：OpenAPI/生成 DTO/事件契约、CI/覆盖率/Bundle/E2E 资源档位、Secret、平台
  MySQL、RAGFlow v0.26.4、备份、生产构建/回滚、全部 ShellCheck 与 `git diff --check` 通过。
  隔离 API/Worker/Preview、两次失败和成功的无头 Chromium、Trace/截图、模型/会话/认证测试数据及
  `18201/18281` 端口已精确清理；稳定 32 GiB real 栈和既有固定镜像/Volume 保留复用，没有遗留
  重复业务镜像或浏览器进程
- 遗留：无；下一任务 S10-07H 左侧历史会话与恢复语义
...
- 日期：2026-07-22
- 提交：本任务提交（见 Git 历史）
- RED：先用正式 Uvicorn/MySQL 回归建立两个员工会话并继续较早会话；旧列表仍按创建时间返回较新
  会话且没有员工名称。组件回归同时证明全局壳层没有“历史会话”区域、点击无法加载任意深页详情，
  员工会话只在首屏员工列表中查绑定对象，重新进入时不会读取员工当前默认模型
- 后端与迁移：新增历史投影和正式 `GET /api/v1/conversations/{id}`，列表使用租户内员工外连接一次
  返回标题、来源与员工名称；Alembic `20260722_0021` 把会话无筛选、来源、员工和标题组合索引切到
  `updated_at DESC, id DESC` 的稳定 keyset。历史查询/详情/删除从会话命令 Router 拆入独立
  `conversation_history.py`，保持职责预算，不提高上限掩盖增长
- 页面与恢复：主导航下方增加全局 `ConversationHistory`，提供加载、空态、失败重试、分页、当前项、
  通用/员工归属和正式二次确认删除；会话工作区移除重复局部历史栏。点击历史通过独立详情接口恢复
  完整消息；员工 ID 以会话权威数据为准并直接读取员工详情，恢复员工当前默认模型，通用会话恢复
  持久化模型。续聊成功后立即刷新详情缓存，避免 Query 新鲜期把旧模型带回页面；员工资源失效时
  关闭输入并提供返回通用 AI 的明确出口
- 生产同路径：隔离 `18201/18281` 使用正式 Uvicorn、项目 MySQL、独立 Worker、生产 Build/Preview、
  Deep Agents 与真实百炼，全程无头 `chromium-headless-shell`。员工链创建 11 条历史并从左侧加载
  下一页，临时切到平台模型获得真实回复，离开后点击历史恢复完整消息且选择器回到员工当前默认
  模型，`1 passed (12.7s)`；通用链以两个模型完成两轮真实回复，从模型页点击历史后恢复最后持久化
  模型、两轮消息并从侧栏删除，修复后 `1 passed (5.2s)`
- 失败闭环：首轮通用真实验收确认服务端已持久化第二轮 `qwen-plus`，但详情 Query 仍保留首轮快照，
  10 秒新鲜期内历史重入不发请求；新增发送后精确失效详情缓存的回归并复验通过。首次权威全量还
  捕获会话 Router 达到 262 行、超过 240 行职责预算；拆出历史 Router 后原架构失败定向 30 通过、
  权威全量复验全绿，没有放宽门槛或靠重跑掩盖
- GREEN：后端权威覆盖率全量 `691 passed, 13 skipped in 191.63s`；总体行 `91.23%`、分支
  `73.07%`、核心行 `93.58%`、核心分支 `77.50%`。Ruff、322 文件格式、严格 Mypy 300 个源/测试
  文件、uv lock 与 85 包安全审计通过。前端 `27 files / 128 tests`，行 `87.06%`、分支 `76.34%`、
  函数 `87.16%`、语句 `85.78%`；ESLint、TypeScript、生产 Build、冻结安装和零高危审计通过，
  `/chat` 首次 JS 图 `1,490,392 bytes` 低于固定 1.5 MB 预算
- 工程门禁与清理：OpenAPI/生成 DTO、CI/覆盖率/Bundle/E2E 资源档位、Secret、平台 MySQL、
  RAGFlow v0.26.4、备份、生产构建/回滚、全部 ShellCheck、Bash 语法与 `git diff --check` 通过。
  正常开发库已迁移到 `0021`，稳定 API/Worker/前端均使用新代码且 real/RAGFlow 健康；隔离进程、
  无头浏览器、模型/员工/会话/认证数据和临时端口已清理，固定 32 GiB 栈、镜像与 Volume 保留复用
- 遗留：无；下一任务 S10-07I 工作流 AI 对话节点执行目标

### S10-07I 工作流 AI 对话节点执行目标

- 状态：✅ 已完成
- 日期：2026-07-22
- 提交：本任务提交（见 Git 历史）
- RED：先把 AI 对话节点改为必须保存互斥的 `employee` 或 `model` 执行目标；旧领域、API、MySQL、
  工作流设计器和运行摘要均不认识该目标，员工删除也不会感知工作流引用。真实运行回归进一步锁定
  模型目标必须走正式模型适配器，员工目标必须走当前员工配置、知识库、授权工作流、Deep Agents
  和当前默认模型，未知、停用、跨租户、空目标及重复/中断模型流均须关闭失败
- 后端与迁移：Alembic `20260722_0022` 新增租户隔离的 `workflow_ai_chat_targets`，用互斥检查、员工/
  模型复合外键和索引保存引用，并为既有 AI 节点回填当前租户平台默认模型；运行记录增加不可漂移的
  实际目标/模型 JSON 快照。`WorkflowAiTargetDirectory` 在保存和运行时统一解析当前资源，保存持有
  资源锁并验证租户、启用状态与引用；员工删除新增工作流引用阻断。LangGraph 节点在模型目标时直接
  流式调用百炼，在员工目标时继承系统提示、知识库和授权工作流并进入 Deep Agents，嵌套工具仍按
  正式手动触发语义运行，不伪造会话来源
- 页面与协议：工作流 AI 对话节点只列当前租户已创建的数字员工和已启用模型，目标互斥且未选择时
  禁止保存；运行面板展示实际目标、模型名称和百炼标识。OpenAPI、生成 DTO、持久事件 Schema 和
  SSE 投影同步增加 `ai_targets`；工作流画布、检查器和运行面板改为按需加载，使 `/workflows` 首次
  JS 图保持在 `1,495,622 bytes`，没有提高 1.5 MB 固定预算
- 生产同路径：隔离 `18201/18281` 使用正式 Uvicorn、项目 MySQL、独立 Worker、前端正式入口和
  无头 `chromium-headless-shell`，从页面拖拽 AI 节点、选择已创建模型、保存、刷新恢复并手动运行，
  真实 LangGraph→百炼回复及运行面板/刷新后的目标快照均通过，最终 `1 passed (11.3s)`；设计器
  单独复验 `1 passed (7.3s)`。员工目标通过正式 HTTP 创建和手动运行进入
  LangGraph→Deep Agents→百炼并验证快照，`1 passed (13.71s)`；员工对话触发授权工作流的真实
  RAGFlow/Deep Agents/百炼链 `1 passed (6.89s)`，四条链均未用 Mock 或进程内客户端冒充验收
- 失败闭环：首次真实运行已在 MySQL 保存正确目标快照，但直播运行面板缺失目标，定位为跨进程持久
  SSE 序列化漏传 `ai_targets`；补齐事件序列化与持久 Broker 重放回归后，直播和刷新结果一致。
  最终构建又发现工作流路由超出预算约 5 kB，拆分并懒加载画布/检查器/运行面板后恢复门禁。严格
  Mypy 还捕获两个测试替身没有跟进观察者新协议，补齐显式目标事件方法后 303 个源/测试文件全绿
- GREEN：启用真实百炼和 RAGFlow v0.26.4 的后端权威覆盖率全量 `719 passed in 296.18s`，无跳过；
  总体行 `91.54%`、分支 `73.93%`、核心行 `93.94%`、核心分支 `78.70%`。Ruff、326 文件格式、
  严格 Mypy 303 个源/测试文件、uv lock 与 85 包安全审计通过。前端 `27 files / 130 tests`，行
  `86.72%`、分支 `75.24%`；ESLint、TypeScript、生产 Build、六路由 Bundle、零高危审计及
  OpenAPI/生成 DTO/事件 Schema 漂移全部通过
- 工程门禁与清理：CI/覆盖率/Bundle/E2E 资源档位、Secret、ShellCheck、Bash 语法和
  `git diff --check` 通过；正常开发库已迁移到 `0022`，稳定 API/Worker/前端已使用新代码且
  real/RAGFlow 健康。隔离 API/Worker/前端、无头 Chromium、模型/员工/工作流/认证验收数据和
  `18201/18281` 端口已精确清理；用户既有 RAGFlow 数据、固定 32 GiB 栈、镜像与 Volume 保留复用
- 遗留：无；下一任务 S10-07J 知识库批量选择与拖拽上传

### S10-07J 知识库批量选择与拖拽上传

- 状态：✅ 已完成
- 日期：2026-07-22
- 提交：本任务提交（见 Git 历史）；全量门禁发现并独立修复的系统 Python 兼容问题为 `471e0eb`
- RED：先增加正式解析重试协议/API、React 批量队列和生产同路径 E2E。旧后端没有
  `retry_document`，定向集成测试得到 `2 failed, 44 passed`；旧前端不存在
  `KnowledgeUploadQueue`，目标组件测试按模块缺失失败。没有把 Mock 上传或直接调用适配器当作跨端完成证据
- 后端与协议：知识服务按当前租户确认知识库和文档归属，只允许失败文档重试；RAGFlow 适配器通过
  `/api/v1/datasets/{id}/chunks` 正式公开入口重新触发解析，并把不存在、非失败状态和结果未知分别映射为
  404、409、502。新增 `knowledge.document.retry_started` 审计动作、正式 202 API、OpenAPI 与生成 DTO；
  API 不接触 RAGFlow Token，浏览器也不直连 `19380`
- 页面：知识库上传区支持文件选择器多选和原生拖拽，单队列最多 20 份、单文件 20 MiB，仅接受
  TXT/Markdown/PDF/DOCX；同名同大小文件在已解析列表或当前队列中稳定去重。队列展示等待、上传、已上传、
  解析、完成和失败，上传请求并发上限为 2；单项失败可只重试该项，成功项不回滚、不重新上传，队列忙碌时
  禁止切换或删除当前知识库
- 生产同路径：隔离 `18201/18281` 使用正式 Uvicorn、项目 MySQL、独立 Worker、生产 Build/Preview、
  真实 RAGFlow v0.26.4、Deep Agents 和百炼，全程为 `chromium-headless-shell`。页面一次拖入 S10-07D 的
  12 份 TXT/Markdown/PDF/DOCX，验证请求并发不超过 2、全部解析完成、重复文件不发请求；随后上传独立
  DOCX，经 RAGFlow 正式接口中断解析，由页面调用平台重试 API 恢复完成。再从页面创建绑定该知识库的
  数字员工并提问，真实回答同时区分 18、12、6 家门店并保留 `01-product-blueprint.md` 与
  `04-pilot-acceptance.docx` 引用，最终 `1 passed (34.1s)`
- 失败闭环：首轮 E2E 发现浏览器重复拖拽会生成不同 `lastModified`，改用名称和大小作为稳定指纹并补回归；
  第二轮使用 1.3 MiB 重复文本生成 2260 个分块，百炼嵌入接口按真实限制拒绝单批超过 10 条，改用已经
  证明可解析的 DOCX 内容保留同一中断/重试语义。该异常还令 RAGFlow API 留下未关闭事务，只精确重启
  `common-agent-ragflow-api` 后恢复，未降低并发或裁剪验收范围。末轮功能已通过，只因同一来源多块引用让
  Playwright 严格定位歧义失败，断言收口到最后一条助手消息的引用区后通过
- GREEN：启用真实百炼和 RAGFlow 的后端权威覆盖率全量 `723 passed in 288.69s`，无跳过；总体行
  `91.46%`、分支 `73.70%`、核心行 `93.47%`、核心分支 `77.35%`。Ruff、327 文件格式、严格 Mypy
  304 个源/测试文件、uv lock 与 85 包安全审计通过。前端 `28 files / 135 tests`，行 `86.93%`、
  分支 `75.11%`、函数 `86.05%`、语句 `85.28%`；ESLint、TypeScript、生产 Build、六路由 Bundle、
  零高危审计及 OpenAPI/生成 DTO 漂移全部通过，覆盖率缺口通过补齐空文件和 20 份上限测试关闭，未降门槛
- 工程门禁与清理：CI/覆盖率/开发入口/real/32 GiB 资源/Bundle/E2E 档位/Secret、平台 MySQL、
  RAGFlow、备份、生产构建/回滚、ShellCheck、Bash 语法和 `git diff --check` 通过；`datetime.UTC`
  不兼容 macOS Python 3.9 的既有脚本问题已单独修复并提交。所有隔离 API/Worker/Preview、无头浏览器、
  批量知识库/员工/会话/认证数据和失败产物已清理，失败产物可从系统废纸篓恢复；用户既有知识库、固定
  32 GiB 栈、镜像和 Volume 未删除。稳定 `18200/18280` 已换用新代码，RAGFlow v0.26.4、Worker 和页面健康
- 遗留：无；下一任务 S10-07M 全仓架构、性能与代码结构 Review 及整改

### S10-07M 全仓架构、性能与代码结构 Review 及整改

- 状态：✅ 已完成
- 日期：2026-07-22
- 提交：每个问题均已独立提交并推送；整改提交为 `b692824`、`1ff6026`、`c1b7433`、
  `a51efb8`、`d538549`、`8b878fa`、`cac11aa`、`dfaba6a`、`7ada17a`、`6fa1d87`、
  `5f5e629`、`a0a1568`、`50c0440`、`c5a98f7`、`6581a1a`，验收隔离与正式入口修正为
  `d57ffc4`、`a9df5fd`、`9e2a42c`
- Review 范围：逐层检查领域、应用、端口、适配器和 API 依赖方向及模块职责；复核 MySQL 查询数、
  事务边界和 N+1，异步并发、持久 Worker、租约与事件序列，百炼/RAGFlow/Deep Agents 关闭、超时和
  部分装配清理；复核 React Query 缓存、按需请求、上传队列、六路由渲染图和包体；复核 OpenAPI、
  SSE 事件 Schema、生成 DTO、租户隔离、凭据扫描、依赖安全与失败可定位性。13 项架构职责门禁和
  5 项依赖方向门禁保持关闭失败，没有放宽复杂度、文件体量、覆盖率或包体预算
- 问题台账与整改：复用工作流重复 AI 目标校验并并发执行独立知识库引用校验；把员工授权工作流从
  N+1 改为一次批量读取，并让空员工页面按需加载知识库、工作流和模型选项；通用会话不再请求员工
  工作流数据。会话与工作流事件改为拒绝重复、倒序和跳号序列。关闭链在单项失败、部分装配失败、
  多缓存模型失败和非幂等 Deep Agents 流场景下仍释放全部已取得资源。冷启动 Mypy 不再依赖模块偶然
  属性。真实浏览器进一步发现文件选择器的实时 `FileList` 在 React 状态更新前被清空，以及员工编辑
  后详情缓存未同步；均先以稳定 RED 复现，再分别修复并定向回归
- 定向与性能证据：工作流批量读取固定为根、节点、边 3 条 MySQL 查询且保持输入顺序；知识库引用
  校验最大 8 路有界并发、确定性返回并等待全部任务收口；员工空页不再提前发出三类引用目录请求；
  通用历史会话不请求运行或工作流列表。资源关闭、事件缺口、上传实时列表和员工详情缓存回归全部
  先失败后通过。六路由首次 JS 图分别为 `/chat` 1,312,551、`/employees` 1,304,708、
  `/knowledge-bases` 1,301,638、`/workflows` 1,496,341、`/model-configurations` 1,294,548、
  `/audit-events` 1,294,355 bytes，最大单 chunk 178,410 bytes，均低于固定预算
- 权威 GREEN：最后一次代码变更后启用真实百炼和 RAGFlow v0.26.4 运行后端全量
  `746 passed in 304.39s`，无跳过；总体行 `91.54%`、分支 `74.43%`、核心行 `93.80%`、核心
  分支 `78.88%`。Ruff、330 文件格式、严格 Mypy 307 个源/测试文件、uv lock 和 85 包安全审计
  通过。前端 `28 files / 142 tests`，行 `87.17%`、分支 `76.19%`、函数 `86.22%`、语句
  `85.70%`；ESLint、TypeScript、冻结安装、生产 Build、npm 高危审计、六路由 Bundle 和
  OpenAPI/生成 DTO/事件 Schema 漂移全部通过
- 生产同路径：使用隔离 MySQL 数据库、正式 Uvicorn、独立持久 Worker、正式 React 页面/API/SSE、
  RAGFlow v0.26.4、Deep Agents、LangGraph 和真实百炼，全程无头 Chromium。最终用户链从页面创建
  知识库、用系统文件选择入口上传并等待真实解析，创建绑定知识库的员工完成带引用的两轮对话，从
  左侧历史会话恢复，拖拽保存并手动运行工作流，再编辑员工授权工作流并从恢复会话触发、查看详情，
  `1 passed (25.1s)`；浏览器未直连 RAGFlow 或百炼。审查中各专项真实会话、员工、工作流和 RAGFlow
  链也均从正式入口复验，不以 Mock、进程内客户端或日志断言替代完成验收
- 工程门禁与清理：CI、覆盖率、demo/real 入口、32 GiB 资源、平台 E2E 档位、Secret、RAGFlow、平台
  MySQL、备份恢复、生产构建/回滚、ShellCheck、Bash 语法和 `git diff --check` 全部通过。共享
  RAGFlow 只隔离 `common-agent-q6-04-knowledge-*` 测试命名空间，用户既有知识库未修改；成功链精确
  删除 1 个工作流和 1 个知识库并重置隔离认证/数据库，`18210/18290`、`18211/18291` 及无头浏览器
...
- 浏览器安全头整改：生产 TLS Edge 对 HTML、静态资源、API、SSE 和错误统一增加一年 HSTS、同源
  ...
  镜像已精确清理
- 生产路由缓存门禁整改：生产演练复现工作流标题先于懒加载运行面板出现，旧用例随即清空请求记录，
  将仍在完成的首次 `WorkflowRunPanel` 请求误判为二次导航重复加载；测试现等待“手动运行面板”正式
  区域可见后再进入缓存验证，不用固定等待或放宽断言掩盖竞态。原失败用例与完整生产浏览器套件
  复验 `4 passed (6.6s)`，正式域名、React 页面、FastAPI/MySQL、请求体边界和安全响应头均走同一路径
- 正式域名入口整改：生产 Edge 现把经过格式与标签边界校验的唯一正式域名写入 `server_name`，并对
  任意非正式 Host 返回 421；Edge 自身健康检查也必须携带正式 Host，避免探针绕过入口规则。生产
  浏览器通过本地域名映射访问 `https://common-agent.test:18443`，API loopback 传输显式携带正式 Host
  并把服务端签发的 Secure/HttpOnly 会话 Cookie 同步到页面域名。首轮分别复现 macOS 系统代理劫持
  本地域名和 loopback Cookie 归属错误，现仅在本地域名 E2E 使用 Chromium `direct://` 与解析规则，
  不改产品认证语义。真实 TLS Edge 对伪造 Host 返回 421，完整套件 `4 passed (6.6s)`，来源头限流、
  蓝绿切换、故障注入和回滚全部通过；临时容器、网络、MySQL Volume、TLS/认证状态、浏览器和四张
  release 镜像已清理，稳定 MySQL、RAGFlow 与正式候选镜像保留
- 路径穿越入口整改：使用保留原始 URI 的 `curl --path-as-is` 从正式 TLS 域名入口复现编码点段在
  Nginx 规范化后落入 SPA fallback 并错误返回 200；Edge 现于规范化前拒绝原始、混合和百分号编码
  点段。生产演练固定保留原始 URI，要求返回 400/404 且响应不得出现系统文件内容；最后一次完整
  蓝绿演练中该门禁与正式浏览器套件、故障注入、切换和回滚一并通过
- 权限与输入攻击矩阵：正式 TLS 域名、React 页面和公开 API 现覆盖未认证读写、生产 Cookie 属性、
  CSRF 缺失/错误、跨站来源、错误 Content-Type、畸形 JSON、未知字段、超长搜索、非法 UUID 五次
  连续请求、租户选择冲突、SQL-like 搜索、TRACE、存储型 hostile 文本渲染、Viewer 页面只读和真实
  同源写入拒绝、跨租户读取及越权组织创建。基础套件固定先执行 `4 passed (6.7s)`，有状态攻击套件
  随后独立执行 `2 passed (3.3s)`；同一隔离生产栈继续完成来源头限流、蓝绿切换、API 故障注入和
  代码回滚。期间用失败结果校正生产 `__Host-` Cookie、Nginx TRACE 405 语义及有状态用例顺序，不以
  Mock、Test Harness、进程内客户端或日志断言替代完成证据
- TLS Edge 首轮容量基线：新增可复用的 k6 正式入口，使用生产 Cookie 和真实租户上下文，以固定
  arrival rate 在 60 秒内轮询员工、会话、工作流和模型四类 MySQL 查询；关闭失败门禁为错误率低于
  0.1%、p95 低于 500 ms、p99 低于 1 s 且不得丢迭代。首次执行由 Homebrew k6 2.1 移除旧
  `--hosts` 参数而在发流量前失败，域名映射改入脚本 `options.hosts` 后从完整隔离生产入口重跑；
  最终 1,501 次容量请求全部 200、错误和丢迭代均为 0，p95 6.82 ms、p99 8.47 ms、最大 64.08 ms。
  同轮无头浏览器基础/攻击套件分别 `4 passed (6.8s)`、`2 passed (4.0s)`，随后来源限流、蓝绿切换、
  API 故障注入与真实代码回滚均通过。该结果只完成 25 次业务迭代/秒的首轮读容量基线，不冒充 SSE
  长连接、并发写入、Worker 积压、故障 soak 或最终 SLO/告警完成证据
- SSE 长连接容量门禁：新增标准库正式压测入口，从 `127.0.0.1` 连接但使用正式
  `common-agent.test` SNI/Host 与本地 CA 校验，经生产 Cookie、真实租户、FastAPI/MySQL 持久事件
  Broker 和 Nginx TLS Edge，以 16 路/秒受控爬升到 128 路并完整保持 360 秒；关闭失败条件固定为
  128 路全部建立、截止时全部存活、意外断连为 0、单连接握手 p95 低于 500 ms、主动关闭后
  `requests_in_flight` 回到 0。Nginx SSE 读取超时由 300 秒提高至 600 秒，使验收窗口真实跨过旧边界
- SSE RED/GREEN：静态/单元测试先以压测入口缺失和模块加载失败建立 RED；首次正式运行又发现攻击
  套件创建的无模型租户可能排在首位、DELETE 清理漏带 Origin/CSRF，以及同一毫秒突发 128 路会把
  单 API/MySQL 校验路径排队时间推至 p95 1.62 秒。修正为遍历可用租户、完整安全头、整批复用 TLS
  上下文和显式 16 路/秒爬升后，7 项回归、Ruff 与生产静态契约全绿，500 ms 阈值未放宽
- SSE 正式结果：最终完整隔离双节点演练得到 `established=128`、`alive_at_deadline=128`、
  `unexpected_disconnects=0`、握手 p95 `23.36 ms`、`requests_in_flight=0`，临时会话和员工删除成功；
  同轮 k6 为 1,500/1,500、错误和丢迭代均为 0、p95 `8.34 ms`、p99 `9.40 ms`，Chromium 基础/攻击
...
- SLO、资源预算与告警 RED/GREEN：先让策略、评估器、资源采样器、运行时范围及重启/OOM 精确诊断
  的单元/静态契约因入口缺失或证据不完整按预期失败，再实现版本化 `slo-policy.json`、严格 JSON
  评估器和每 5 秒 loopback TLS/Docker 采样器。最终 38 项生产压测定向回归通过；策略固定 99.9%
  可用性、API 错误率不高于 0.1%、p95/p99 不高于 500/1,000 ms、SSE 128 路、Worker 24 条容量与
  8 条崩溃恢复、业务节点 6C12G、RAGFlow 节点 6C24G，并为可用性、错误预算、延迟、SSE、积压、
  恢复、两节点资源、容器重启/OOM、备份 RPO 和恢复演练时效定义 11 类带处置说明的关闭失败告警
- 最终生产同路径：全新隔离租户先由正式页面核验并使用播种的“平台默认模型” `qwen-plus`，再完成
  知识库、员工、带引用两轮会话、历史恢复、手动工作流、员工授权触发与审计；Chromium 基础 4/4、
  MVP 1/1、审计 1/1、权限与输入攻击 2/2。k6 共 1,502 请求、错误率与丢迭代 0、p95 `8.5905 ms`、
  p99 `10.09468 ms`；SSE 128/128 保持 360 秒、意外断连 0、握手 p95 `23.118792 ms`、关闭后
  in-flight 0；Worker 容量 24/24 在 `8.690468 s` 排空，崩溃后 8/8 在 `71.417449 s` 接管且
  `attempts >= 2`
- 最终 SLO 结果：83 个资源样本全部健康，可用性 `1.0`，活动告警为空且所有容器无重启/OOM；业务
  节点峰值 CPU `0.9497` 核、内存 `1,003,540,709` 字节，RAGFlow 节点峰值 CPU `0.6546` 核、内存
  `6,545,463,051` 字节，均低于预算。随后来源头攻击、绿槽切换、active API 故障注入、蓝槽代码
  回滚和回滚后验证继续全绿，`infra/production/manage.sh drill` 最终退出码 0
- 灾备终验：另行执行正式 `infra/backup/manage.sh drill`，由 Chromium 在源环境播种业务数据，形成
  AES-256-GCM 冷快照并销毁源服务，再恢复到独立空 MySQL/RAGFlow Volume；恢复耗时 `60 s`，正式
  页面恢复核验 1/1，真实 RAGFlow 外部引用 1 条有效，满足 120 分钟 RTO 与 90 天演练周期。运行时
  SLO 门禁和真实备份演练保持独立，避免用伪造备份时间戳冒充灾备证据
- 最终工程门禁：后端权威全量 `771 passed, 13 skipped in 279.65s`；overall 行/分支覆盖率
  `91.14%/73.53%`、core `93.28%/77.59%`；严格 Mypy 316 个文件、Ruff 检查及 338 个文件格式通过，
  后端 85 个包漏洞审计为 0。前端 28 个文件 142 项测试、覆盖率、ESLint、TypeScript、冻结构建、
  六路由包体和 422 个包高危审计通过；OpenAPI/生成 DTO/事件 Schema、CI/覆盖率/入口/安全扫描契约、
  28 个 Shell 脚本的 ShellCheck/Bash 语法、Secret、Semgrep、Trivy 依赖/Secret/IaC 与生产静态契约
  全部通过。13 个 skip 均要求显式外部环境，而同轮正式生产与灾备演练已覆盖真实 RAGFlow/百炼链
- 清理与遗留：生产和灾备 trap 已删除本轮容器、网络、临时 MySQL/RAGFlow Volume、TLS、认证状态、
  状态目录和无头浏览器/压测进程；稳定平台 MySQL 与 RAGFlow 恢复验收前停止状态。S10-08 本机范围
  无遗留且本任务提交见 Git 历史；尚未采购、创建或发布远程资源，下一步必须由用户明确发出“部署”
  指令后才进入远程首次发布

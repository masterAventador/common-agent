...
| 本地服务 | `✅` 供用户联调的正式开发 API `18200`、Worker、前端 `18280`、平台 MySQL 与 RAGFlow v0.26.4 已换用本轮新代码并保持运行；正常开发库位于 `20260722_0022`，隔离 API/Worker/前端、无头浏览器、批量知识库/员工/会话测试数据与失败产物均已清理，稳定 Volume、0600 Token、现有 Demo Key、升级前认证备份与固定镜像保留 |
| 后续整改 | `🟨` Wave 7、Wave 8、Wave 9、S10-01 至 S10-07L（含 S10-07J）已完成；下一任务为 S10-07M 全仓架构、性能与代码结构 Review，完成后进入 S10-08；尚未采购、创建或发布任何远程资源 |
...
| S10-08 | 生产安全、性能与最终验收 | 执行依赖/SAST/Secret/容器扫描、权限与输入攻击测试、并发/容量/长连接/故障恢复压测；定义并满足 SLO、资源预算和告警；从新租户通过正式浏览器完成模型→知识库→员工→会话→工作流并验证审计、备份与回滚 | S10-07M | 🚧 实现中 |
...
S10-07I 已完成可选员工或模型的工作流 AI 对话节点；下一步交付知识库批量选择/拖拽上传。
全部功能完成后由 S10-07M 执行全仓架构、性能和代码结构 Review 与逐项整改，
通过后再进入 S10-08 生产安全、性能、SLO/告警与最终验收。所有后续任务仍

...
| 本地服务 | `✅` 临时前后端均已停止；平台 MySQL 与移除本地模型后的 RAGFlow 五服务保留在独立 `colima-common-agent-dev` 稳定栈供后续复用 |
| 后续整改 | `🟨` R8-00 已完成；CI、平台自有协议、可观测性、运行态内存、代码拆分、开发体验、资源优化、数据管理和生产化继续按 Wave 7-10 推进，MVP 完成事实不变 |
...
- 版本化百炼 Demo Key 的历史授权继续有效到 S10-01；S10-01 必须迁移到受控密钥注入、轮换旧
  Key，并证明 Git 历史、构建、日志和前端均不再扩散可用凭据。
...
基础验收；这只是暂定运行值，不代替专项峰值结论。R8-04 必须把完整正式链路峰值压到不高于
25 GiB，保留至少约 20%/6 GiB 可用余量；连续完成冷启动、真实文档解析/检索、两轮会话、工作流
和 30 分钟稳定性测试，无 OOM、持续 Swap 压力、容器重启、健康抖动或检索质量下降后，才能确认
32 GiB 为长期 real 默认值，否则必须按实测上调。日常 Demo 仍使用 8-12 GiB 轻量模式，避免在
64 GiB 主机运行其他任务时长期占住一半内存；不得只调低 `mem_limit` 冒充优化。
...
| D8-01 | 全新克隆一键 Demo | 提供统一的 `doctor/setup/up/status/stop/clean` 开发入口；检查并冻结安装 uv/pnpm 依赖、使用轻量平台 MySQL 启动 Demo 前后端、显示访问地址并精确清理；从无 `.venv`/`node_modules` 的临时克隆完成两轮 Demo 会话，失败给出可操作信息，当前 64 GiB 电脑不启动 48 GiB profile | R8-00 | ⬜ 未开始 |
...
| ID | 任务 | 交付物与完成定义 | 依赖 | 状态 |
| --- | --- | --- | --- | --- |
| U9-01 | 资源删除策略与后端 API | 为会话、数字员工、知识库和工作流定义引用检查、级联/拒绝策略、外部副作用不确定语义和幂等删除；实现正式 API、MySQL/RAGFlow 事务边界及失败恢复，删除前后不留下悬空绑定、运行来源或文档 | R8-04 | ⬜ 未开始 |
| U9-02 | 删除 UI 与真实验收 | 四入口提供目标清晰的删除确认、引用阻断说明和完成/失败状态；无头浏览器验证创建→引用→阻断/解绑→删除→刷新消失，不能用测试清理器代替用户链路 | U9-01 | ⬜ 未开始 |
| U9-03 | 列表分页、搜索与稳定排序 | 为会话、员工、知识库、工作流和运行摘要建立统一游标或明确分页契约、服务端搜索和稳定排序；前端保持筛选/页游标并处理新增/删除并发变化；大数据集下无全表/N+1 和重复/遗漏项 | U9-02 | ⬜ 未开始 |
...
| ID | 任务 | 交付物与完成定义 | 依赖 | 状态 |
| --- | --- | --- | --- | --- |
| S10-01 | 凭据治理与 Demo Key 轮换 | 停止版本化可用百炼 Key，改用本机/CI/部署环境受控 Secret 注入；轮换并作废旧 Key，扫描 Git 历史、日志、Trace、构建和前端产物；错误与诊断只显示配置状态，不显示值 | U9-03 | ⬜ 未开始 |
| S10-02 | 身份认证与安全会话 | 实现注册策略、登录/退出、安全 Cookie 或等价会话、CSRF/重放/暴力尝试防护和凭据恢复边界；所有写接口默认拒绝未认证访问，真实浏览器覆盖成功、过期、撤销和攻击失败路径 | S10-01 | ⬜ 未开始 |
| S10-03 | 组织、租户与 RBAC 隔离 | 为员工、知识库引用、会话、工作流和运行增加租户归属；定义最小角色/权限和跨租户拒绝；迁移、唯一约束、查询、缓存/事件命名空间和外部 RAGFlow 数据均隔离，越权测试覆盖 REST/SSE/工具调用 | S10-02 | ⬜ 未开始 |
| S10-04 | 审计与安全事件 | 对登录、配置、绑定、上传、删除、工作流运行、权限拒绝和凭据操作写入不可篡改/可追溯审计记录；支持按租户、操作者、资源和时间查询，正文与凭据脱敏，保留策略和容量上限明确 | S10-03,H7-06 | ⬜ 未开始 |
...
R8-00 已完成：RAGFlow 官方源码以 submodule 固定且保持未修改，知识库 embedding/rerank 已统一
使用阿里百炼，本地模型退场，既有数据预检为空，隔离中文索引重建和 32 GiB 基础资源门禁通过。
下一任务 D8-01 提供全新克隆的一键 `demo-light` 与 8-12 GiB 门禁，随后按 D8-02、D8-03、
R8-04 证明 Demo 持久语义、real 一键诊断和 32 GiB 长期峰值/soak。D8-03 完成后进入 H7-01，

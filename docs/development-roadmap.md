...
> 文档性质：项目开发进度唯一执行台账  
> 建立日期：2026-07-19  
> 当前阶段：S10-07F 数字员工默认模型已完成，下一任务为 S10-07G
...
| 产品代码 | `✅` 知识库、数字员工、连续会话、工作流设计/运行及两类触发的 MVP 正式链路已完成；数字员工默认模型已使用租户模型配置、正式引用和逐员工百炼路由 |
| 本地服务 | `✅` 供用户联调的正式开发 API `18200`、Worker、前端 `18280`、平台 MySQL 与 RAGFlow v0.26.4 保持运行；正常开发库已迁移到 `20260722_0019`，本轮隔离 API/Worker/前端、无头浏览器和测试数据已清理，稳定 Volume、0600 Token、现有 Demo Key、升级前认证备份与固定镜像保留 |
| 后续整改 | `🟨` Wave 7、Wave 8、Wave 9、S10-01 至 S10-07F、S10-07K 与 S10-07L 已完成；S10-07G 至 S10-07J 与 S10-07M 已排入并将在 S10-08 前逐项交付；尚未采购、创建或发布任何远程资源 |
...
| S10-07G | 通用会话、自动建会话与逐轮模型切换 | 点击 AI 会话直接进入无需数字员工的空白通用对话；用户无需预建会话，首次发送时由正式入口原子创建会话并提交首轮。输入框底部只显示用户已创建且启用的模型，可在每轮发送前切换；会话持久化通用/数字员工来源和实际逐轮模型，通用会话恢复上次选择，员工会话中的临时模型切换只影响当次/当前打开期间，不改员工默认模型 | S10-07F | ⬜ 未开始 |
...
S10-07F 已完成数字员工默认模型、引用安全和逐员工百炼路由；下一步从 S10-07G 开始依次交付无需
员工且首次发送自动创建的通用会话与逐轮模型切换、左侧历史会话恢复、可选员工或模型的工作流 AI 对话节点，以及知识库

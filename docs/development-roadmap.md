...
> 文档性质：项目开发进度唯一执行台账  
> 建立日期：2026-07-19  
> 当前阶段：S10-07H 左侧历史会话与恢复语义已完成；下一任务 S10-07I
...
| 产品代码 | `✅` 知识库、数字员工、通用/员工连续会话、全局历史恢复、工作流设计/运行及两类触发的 MVP 正式链路已完成；数字员工默认模型、通用会话当前模型与助手逐轮模型快照均使用租户模型配置和正式引用 |
| 本地服务 | `✅` 供用户联调的正式开发 API `18200`、Worker、前端 `18280`、平台 MySQL 与 RAGFlow v0.26.4 保持运行；正常开发库已迁移到 `20260722_0021`，本轮隔离 API/Worker/前端、无头浏览器和测试数据已清理，稳定 Volume、0600 Token、现有 Demo Key、升级前认证备份与固定镜像保留 |
| 后续整改 | `🟨` Wave 7、Wave 8、Wave 9、S10-01 至 S10-07H、S10-07K 与 S10-07L 已完成；S10-07I、S10-07J 与 S10-07M 已排入并将在 S10-08 前逐项交付；尚未采购、创建或发布任何远程资源 |
...
| S10-07I | 工作流 AI 对话节点执行目标 | 工作流 AI 对话节点提供互斥执行目标：选择一个已创建数字员工，或直接选择一个用户已创建且启用的模型；员工目标继承其系统提示、知识库、授权工作流和当前默认模型，模型目标执行通用 AI 对话。服务端保存/编译时校验租户、启用状态和引用，运行摘要记录实际目标与模型；通过正式拖拽编辑、保存、手动运行和员工触发路径完成真实 LangGraph/Deep Agents/百炼验收 | S10-07H | ⬜ 未开始 |
...
S10-07H 已完成全局左侧历史、稳定更新时间排序和通用/员工恢复语义；下一步从 S10-07I 开始依次交付
可选员工或模型的工作流 AI 对话节点，以及知识库
批量选择/拖拽上传。全部功能完成后由 S10-07M 执行全仓架构、性能和代码结构 Review 与逐项整改，

...
> 文档性质：项目开发进度唯一执行台账  
> 建立日期：2026-07-19  
> 当前阶段：S10-07D 至 S10-07J 产品增补任务已完成规划；下一任务为知识库人工验收样本文档
...
| 后续整改 | `🟨` Wave 7、Wave 8、Wave 9、S10-01 至 S10-07C 已完成；S10-07D 至 S10-07J 已按用户最新需求排入并将在 S10-08 前逐项交付；尚未采购、创建或发布任何远程资源 |
...
| S10-07D | 知识库人工验收样本文档 | 在项目内建立独立样本目录与使用说明，提供内容可交叉检索且带唯一事实标记的 TXT、Markdown、PDF、DOCX；二进制文件必须完成结构、文本提取、页数和无窗口渲染检查，样本不得含真实凭据、用户数据或伪装成产品规则 | S10-07C | ⬜ 未开始 |
...
连接或发布任何远程
资源，第一次远程发布仍须用户再次明确授权。用户随后补充的 S10-07D 至 S10-07J 将先建立知识库
人工验收样本，再依次交付用户模型管理、数字员工默认模型、无需员工且首次发送自动创建的通用
会话与逐轮模型切换、左侧历史会话恢复、可选员工或模型的工作流 AI 对话节点，以及知识库批量
选择/拖拽上传。下一任务为 S10-07D；上述任务全部通过后再进入 S10-08 生产安全、性能、SLO/告警
与最终验收。所有后续任务仍

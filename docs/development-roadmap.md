...
| 本地服务 | `✅` S10-01 验收后 demo/real 前后端、平台/RAGFlow 容器和项目专属 Colima 已停止；原生 Volume、0600 Token、现有 Demo Key 与官方镜像保留，下一任务可按需复用 |
...
| S10-02 | 身份认证与安全会话 | 实现注册策略、登录/退出、安全 Cookie 或等价会话、CSRF/重放/暴力尝试防护和凭据恢复边界；所有写接口默认拒绝未认证访问，真实浏览器覆盖成功、过期、撤销和攻击失败路径 | S10-01 | 🚧 实现中 |
...
后端归档和前端生产包由同一指纹门禁扫描。当前进入 S10-02，开始实现身份认证与安全会话。所有后续任务仍

...
> 文档性质：项目开发进度唯一执行台账  
> 建立日期：2026-07-19  
> 当前阶段：S10-07G 已完成；下一任务 S10-07H 左侧历史会话与恢复语义未开始
...
| 产品代码 | `✅` 知识库、数字员工、通用/员工连续会话、工作流设计/运行及两类触发的 MVP 正式链路已完成；数字员工默认模型、通用会话当前模型与助手逐轮模型快照均使用租户模型配置和正式引用 |
| 本地服务 | `✅` 供用户联调的正式开发 API `18200`、Worker、前端 `18280`、平台 MySQL 与 RAGFlow v0.26.4 保持运行；正常开发库已迁移到 `20260722_0020`，本轮隔离 API/Worker/前端、无头浏览器和测试数据已清理，稳定 Volume、0600 Token、现有 Demo Key、升级前认证备份与固定镜像保留 |
| 后续整改 | `🟨` Wave 7、Wave 8、Wave 9、S10-01 至 S10-07G、S10-07K 与 S10-07L 已完成；S10-07H 至 S10-07J 与 S10-07M 已排入并将在 S10-08 前逐项交付；尚未采购、创建或发布任何远程资源 |
...
| S10-07H | 左侧历史会话与恢复语义 | 在现有左侧主菜单下增加分割线和按更新时间稳定排序的历史会话区，显示会话标题及“通用 AI”或数字员工归属；点击直接加载完整历史并可继续对话，刷新、分页/加载更多、删除和资源失效均有明确状态。重新进入员工会话时始终回到该员工当前默认模型，即使此前逐轮临时选择过其他模型；通用会话恢复其持久化模型 | S10-07G | ⬜ 未开始 |
...
S10-07G 已完成无需员工且首次发送自动创建的通用会话与逐轮模型切换；下一步从 S10-07H 开始依次交付
左侧历史会话恢复、可选员工或模型的工作流 AI 对话节点，以及知识库

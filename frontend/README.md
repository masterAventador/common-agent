...
`/employees` 是数字员工正式页面：通过平台 API 查看、新建和编辑数字员工，从当前工作区已启用
模型中选择默认模型，可选绑定一个 RAGFlow 知识库、从正式工作流列表多选允许调用项，并从卡片
进入带 `employee_id` 的聊天入口。员工、模型、知识库与工作流列表独立加载；任一选项服务暂不可用
时只禁用对应字段，已有绑定和授权数量仍明确展示并提供独立重试。已停用模型不进入新的选择，但
编辑员工时可原样保留当前绑定。页面不直接请求 RAGFlow 或百炼，也不包含行业或业务自动化字段。
...
`pnpm test:e2e:employee-model` 使用生产构建、正式 API/Worker/MySQL、Deep Agents 和百炼验证员工
默认模型的创建、编辑切换、刷新、停用保留、新选择过滤、引用删除阻断和真实回复。套件故意把
进程默认模型设为无效标识，避免员工持久化模型未生效时产生假阳性。

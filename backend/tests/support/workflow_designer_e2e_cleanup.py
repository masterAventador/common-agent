...
async def _cleanup() -> tuple[int, int]:
    ...
    try:
        workflows_deleted = await delete_workflows_named(
            database,
            _required("COMMON_AGENT_E2E_WORKFLOW_NAME"),
        )
    finally:
        await database.stop()
    ...
    datasets_deleted = await delete_datasets_named(
        ...
        _required("COMMON_AGENT_E2E_WORKFLOW_KNOWLEDGE_NAME"),
    )
...
def main() -> None:
    ...
    print(f"已清理工作流设计器 E2E 数据: 工作流={workflows_deleted}, 知识库={datasets_deleted}")

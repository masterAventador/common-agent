...
async def _cleanup() -> tuple[int, int]:
    employee_name = _required("COMMON_AGENT_E2E_MVP_EMPLOYEE_NAME")
    ...
    try:
        ...
        workflows_deleted = await delete_workflows_named(
            database,
            _required("COMMON_AGENT_E2E_MVP_WORKFLOW_NAME"),
        )
    finally:
        await database.stop()
    ...
    knowledge_deleted = await delete_datasets_named(
        ...
        _required("COMMON_AGENT_E2E_MVP_KNOWLEDGE_NAME"),
    )
...
def main() -> None:
    ...
    print(f"已清理 MVP 总验收数据: 工作流={workflows_deleted}, 知识库={knowledge_deleted}")
